# ======================================================================
#  ProPhoenix DB Utility Dashboard - v120.0 (Select All + Create DB Fix)
# ======================================================================
...
$form.Text = "ProPhoenix DB Utility Dashboard v120"
...
# --- LIST & LOG (UPDATED FOR SELECT ALL) ---
...
# Inner layout for Select All + List
...
$chkAll.Add_CheckedChanged({ 
    for($i=0; $i -lt $listDBs.Items.Count; $i++){ $listDBs.SetItemChecked($i, $chkAll.Checked) } 
})
...
$btnBackup.Add_Click({
    ...
    if([System.Windows.Forms.MessageBox]::Show("Backup parameters from selected databases?", "Confirm", "YesNo", "Question") -ne "Yes"){ return }
        ...
        try {
            ...
            if ($CmdChk.ExecuteScalar() -gt 0) {
                ...
                if($DT.Rows.Count -gt 0){ 
                    $Exp=@(); foreach($r in $DT.Rows){ $o=[ordered]@{}; foreach($c in $DT.Columns){$o[$c.ColumnName]=$r[$c]}; $Exp+=[PSCustomObject]$o }
                    $Exp | Export-Csv -Path "$TDir\$($DB)_KPIJobs_$TS.txt" -NoTypeInformation -Delimiter "`t"; Log "   + Jobs Saved" "Lime" 
                }
            } else { Log "   - No KPIJobs (Skipped)" "Gray" }

            # Params (Use dbo.Parameter now)
            $CmdChk.CommandText = "SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'Parameter'"
            if ($CmdChk.ExecuteScalar() -gt 0) {
                $SQL = "SELECT CAST(ParamID AS VARCHAR(10)) + ' = ' + ISNULL(CAST(ParamValue AS VARCHAR(MAX)), '') AS ParameterBackup FROM dbo.Parameter WHERE ParamID IN ($GParams) ORDER BY ParamID"
                $DA=New-Object System.Data.SqlClient.SqlDataAdapter($SQL, $CN); $DT=New-Object System.Data.DataTable; $DA.Fill($DT)|Out-Null
                if($DT.Rows.Count -gt 0){ 
                    $OutLines = @(); foreach($R in $DT.Rows){ $OutLines += $R[0] }
                    $OutLines | Out-File "$TDir\$($DB)_GlobalParameters.txt" -Encoding UTF8; Log "   + Params Saved" "Lime" 
                }
            } else { Log "   - No Parameter Table (Skipped)" "Gray" }
            $CN.Close()
...
$btnRestore.Add_Click({
    ...
    if([System.Windows.Forms.MessageBox]::Show("RESTORE data from:`n$SourceFile`n`nINTO selected databases?", "Confirm Restore", "YesNo", "Warning") -ne "Yes"){ return }
        ...
        try {
            ...
            foreach ($Line in $Content) {
                ...
                if ($Parts.Count -eq 2) {
                    ...
                    try { $Cmd=$CN.CreateCommand(); $Cmd.CommandText="UPDATE dbo.Parameter SET ParamValue = '$Val' WHERE ParamID = $ParamID"; $Cmd.ExecuteNonQuery()|Out-Null; $Count++ } catch {}
                }
            }
...
# --- FIXED CREATE DB WITH CATEGORY ---
function Show-NewDBDialog {
    $dbForm = New-Object System.Windows.Forms.Form; $dbForm.Text="Create DB"; $dbForm.Size="400,380"; $dbForm.StartPosition="CenterParent"
    
    $lblC=New-Object System.Windows.Forms.Label;$lblC.Text="Category:";$lblC.Location="20,30";$dbForm.Controls.Add($lblC)
    $cmbCat=New-Object System.Windows.Forms.ComboBox;$cmbCat.Items.AddRange(@("Police","Fire","Phoenix Master","IA","Police CSP","Fire CSP"));$cmbCat.SelectedIndex=0;$cmbCat.Location="120,27";$cmbCat.Width=240;$dbForm.Controls.Add($cmbCat)
    ...
    $inDB=Add-Field "Database" 70 "PhoenixPolice"; $inJID=Add-Field "JurisID" 110 "1000"; $inSt=Add-Field "State" 150 "MA"; $inN=Add-Field "Name" 190 "ProPhoenix"; $inA=Add-Field "Alias" 230 "PNX"
    
    $btn=New-Object System.Windows.Forms.Button;$btn.Text="CREATE";$btn.DialogResult="OK";$btn.Location="120,280";$dbForm.Controls.Add($btn);$dbForm.AcceptButton=$btn
    if($dbForm.ShowDialog() -eq "OK"){return @{DB=$inDB.Text;JID=$inJID.Text;St=$inSt.Text;Nm=$inN.Text;Al=$inA.Text;Cat=$cmbCat.SelectedItem}} return $null
}

$btnCreateDB.Add_Click({ 
    if(!$Script:DBSyncRoot){ [System.Windows.Forms.MessageBox]::Show("Set Utility Path!"); return }
    $d=Show-NewDBDialog; if(!$d){return}
    Toggle $false; Log "Creating Database '$($d.DB)'..." "Cyan"
    try {
        $Folder = $d.Cat
        $TargetDir = Join-Path $Script:DBSyncRoot $Folder
        if (!(Test-Path $TargetDir)) { $Folder = $Folder.Replace(" ", ""); $TargetDir = Join-Path $Script:DBSyncRoot $Folder }
        if (!(Test-Path $TargetDir)) { throw "Folder '$($d.Cat)' not found in Utility path." }

        $ExeFile = Join-Path $TargetDir "PnxDBSync.exe"
        if (!(Test-Path $ExeFile)) { throw "PnxDBSync.exe not found." }
        $XmlPath = Join-Path $TargetDir "PnxAutoNewDBSyn.xml"
        if (!(Test-Path $Script:XmlTarget)) { throw "Missing Template XML" }
        
        [xml]$x = Get-Content $Script:XmlTarget
        $x.PnxPakager.SourceServer.IPAddress = $txtS.Text; $x.PnxPakager.SourceServer.DBName = $d.DB
        $x.PnxPakager.SourceServer.UserName = $txtU.Text; $x.PnxPakager.SourceServer.Password = $txtP.Text
        $x.PnxPakager.SourceServer.JurisID = $d.JID; $x.PnxPakager.SourceServer.State = $d.St
        $x.PnxPakager.SourceServer.JurisName = $d.Nm; $x.PnxPakager.SourceServer.JurisAlias = $d.Al
        $x.PnxPakager.SourceServer.SyncType = "1"
        $x.Save($XmlPath)

        Log "   Launching Utility..." "Gray"
        Start-Process -FilePath $ExeFile -WorkingDirectory $TargetDir -Verb RunAs -Wait
        Log "   ✔ Execution Finished." "Lime"; $btnCon.PerformClick()
    } catch { Log "❌ Creation Failed: $($_.Exception.Message)" "Red"; [System.Windows.Forms.MessageBox]::Show("Error: $($_.Exception.Message)", "Failed") } finally { Toggle $true }
})

# --- INSTALL ---
$RunAppMgr = { param($Mode)
    if (!$Script:AppMgrPath) { [System.Windows.Forms.MessageBox]::Show("App Manager not found!"); return }
