# ======================================================================
#  ProPhoenix DB Utility Dashboard - v111.0 (Parameter Backup Fix)
# ======================================================================
...
$form.Text = "ProPhoenix DB Utility Dashboard v111"
...
# --- ACTIONS ---
$pnlAct = New-Object System.Windows.Forms.TableLayoutPanel; $pnlAct.Dock="Fill"; $pnlAct.ColumnCount=3; $pnlAct.RowCount=1
$pnlAct.ColumnStyles.Add((New-Object System.Windows.Forms.ColumnStyle([System.Windows.Forms.SizeType]::Percent, 33)))
$pnlAct.ColumnStyles.Add((New-Object System.Windows.Forms.ColumnStyle([System.Windows.Forms.SizeType]::Percent, 33)))
$pnlAct.ColumnStyles.Add((New-Object System.Windows.Forms.ColumnStyle([System.Windows.Forms.SizeType]::Percent, 33)))
...
$btnBackup = New-Object System.Windows.Forms.Button; $btnBackup.Text="PARAMETER BACKUP"; $btnBackup.Dock="Fill"; $btnBackup.BackColor=[System.Drawing.Color]::MediumPurple; $btnBackup.ForeColor="White"; $btnBackup.FlatStyle="Flat"; $btnBackup.Font=$fontHeader; $btnBackup.Margin=New-Object System.Windows.Forms.Padding(10,0,10,0); $pnlAct.Controls.Add($btnBackup, 1, 0)
$btnVer = New-Object System.Windows.Forms.Button; $btnVer.Text="CHECK DB VERSIONS"; $btnVer.Dock="Fill"; $btnVer.BackColor=[System.Drawing.Color]::DarkOrange; $btnVer.ForeColor="White"; $btnVer.FlatStyle="Flat"; $btnVer.Font=$fontHeader; $pnlAct.Controls.Add($btnVer, 2, 0)
...
function Toggle($s) {
    $btnCon.Enabled=$s; $btnSync.Enabled=$s; $btnBackup.Enabled=$s; $btnVer.Enabled=$s; $btnCreateDB.Enabled=$s
}
...
$btnBackup.Add_Click({
    ...
    foreach ($item in $listDBs.CheckedItems) {
        ...
        try {
            ...
            # --- FIX: Check if KPIJobs exists ---
            $CmdChk = $CN.CreateCommand()
            $CmdChk.CommandText = "SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'KPIJobs'"
            if ($CmdChk.ExecuteScalar() -gt 0) {
                $DA=New-Object System.Data.SqlClient.SqlDataAdapter("SELECT * FROM dbo.KPIJobs WITH (NOLOCK) WHERE JobName IN ($RMSJobs)", $CN)
            ...
            } else { Log "   - No KPIJobs Table (Skipped)" "Gray" }

            # --- FIX: Check if ParameterName exists ---
            $CmdChk.CommandText = "SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'ParameterName'"
            if ($CmdChk.ExecuteScalar() -gt 0) {
                $DA=New-Object System.Data.SqlClient.SqlDataAdapter("SELECT ParamID, DefaultValue FROM dbo.ParameterName WITH (NOLOCK) WHERE ParamID IN ($GParams)", $CN)
                $DT=New-Object System.Data.DataTable; $DA.Fill($DT)|Out-Null
            ...
            } else { Log "   - No Params Table (Skipped)" "Gray" }
