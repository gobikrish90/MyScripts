...
$Script:btnBackup.Add_Click({
    try {
        if($Script:listDBs.CheckedItems.Count -eq 0){ [System.Windows.Forms.MessageBox]::Show("Select DBs to Backup!", "Warning", "OK", "Warning") | Out-Null; return }
        
        $inputJuris = Show-InputBox "JurisID Required" "Enter JurisID for Backup:" "1000"
        if ([string]::IsNullOrWhiteSpace($inputJuris)) { return }
        if (-not ($inputJuris -match '^\d+$')) {
            [System.Windows.Forms.MessageBox]::Show("JurisID must be a valid number.", "Error", "OK", "Error") | Out-Null
            return
        }
        ...
        Toggle $false; Log "▶ STARTING CONFIGURATION BACKUP FOR JURIS: $inputJuris..." "Cyan"; $TS=Get-Date -Format "yyyyMMdd_HHmm"
        ...
        foreach ($item in $Script:listDBs.CheckedItems) {
            ...
            $cmd.CommandText = @"
            SET NOCOUNT ON;
            DECLARE @JurisID INT = $inputJuris;
...
"@
            ...
            foreach ($job in $tempJobs) {
                ...
                # --- FIX: REVERTED TO SELECT * WITH INDEX 4 ---
...
$Script:btnRestore.Add_Click({
    try {
        ...
        $inputJuris = Show-InputBox "Target JurisID" "Enter Target JurisID for Restore:" "1000"
        ...
        if([System.Windows.Forms.MessageBox]::Show("RESTORE Configuration to Juris: $inputJuris?`n`nAre you sure?", "Confirm", "YesNo", "Warning") -ne "Yes"){ return }
        
        Toggle $false
        Log "▶ RESTORING TO JURIS $inputJuris..." "Orange"
