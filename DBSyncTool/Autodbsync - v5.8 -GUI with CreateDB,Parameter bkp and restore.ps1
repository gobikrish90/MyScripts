# ======================================================================
#  ProPhoenix DB Utility Dashboard - v5.8
# ======================================================================
...
$form.Text = "ProPhoenix DB Utility Dashboard v143"
            ...
            if ($JobID) {
                ...
                foreach ($Row in $DT.Rows) {
                    foreach ($Col in $DT.Columns) {
                        ...
                        if ($Val -match "^\\\\|^[a-zA-Z]:\\") {
                            $ConfigLines += "Folder$fc=$Val"
                            $fc++
                            break # Found the folder column
                        }
                    }
                }
                $ConfigLines += "" # Spacer
...
$btnBackup.Add_Click({
    ...
    foreach ($item in $listDBs.CheckedItems) {
        ...
        try {
            ...
            # 1. Export Params (Smart Table Detection)
            $CmdParam=$CN.CreateCommand(); 
            $CmdParam.CommandText="SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME IN ('Parameter','ParameterName')"; 
            $Tbl=$CmdParam.ExecuteScalar()
            ...
            if($Tbl){
                ...
                if($DT.Rows.Count-gt 0){ 
                    $O=@(); foreach($R in $DT.Rows){$O+=$R[0]}; $O|Out-File "$TDir\$($DB)_GlobalParameters.txt" -Encoding UTF8; Log "   + Params Saved ($Tbl)" "Lime" 
                } else { Log "   - No Matching Params Found" "Gray" }
            } else { Log "   - No Parameter Table Found" "Gray" }
            
            $CN.Close()

            # 2. Export Job Config (Dynamic)
...
$btnRestore.Add_Click({
    ...
    # Auto-Detect associated Config File
    ...
    foreach ($item in $listDBs.CheckedItems) {
        ...
        try {
            ...
            if($Tbl){ 
                ...
                Log "   ✔ Updated $Count params" "Lime" 
            }

            # 2. Restore Job Config (Dynamic)
            if (Test-Path $ConfigFile) {
                $CurJob=""; Get-Content $ConfigFile | ForEach-Object {
                    $Line=$_.Trim(); if($Line -match "^\[(.+)\]$"){$CurJob=$Matches[1]}
                    elseif($Line -match "Instance=(.+)") {
                ...
                Log "   ✔ Job Config Restored" "Lime"
            } else { Log "   - No Config File Found" "Gray" }
