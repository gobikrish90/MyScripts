...
.SYNOPSIS
    ...
    FINAL CORRECTIONS:
    1. CRASH FIX: Removed internal 'Out-File' from helper scripts to prevent "File in Use" and "Drive not found" errors. 
       (Logging is now handled exclusively by the Batch File wrapper).
    2. DETAIL: Cleanup now lists the specific Instance Name and Folder Path for every operation.
    3. STAGE INSTALL: Confirmed "Y" + Enter logic.
#>
...
$GeneratorLogic = {
    ...
    # 1. CLEANUP SCRIPT (Detailed Instance & Path Reporting)
...
foreach (`$basePath in `$proPhoenixBasePaths) {
    ...
    foreach (`$instRoot in `$instanceRoots) {
        # Loop through each Instance (e.g., chpnx862, Live)
...
"@
    ...
    # 2. VERIFICATION SCRIPT (Output Only - No File Writing to prevent locks)
...
"@
        ...
        $FinalName = "${CurrentHostName}_HotfixAutomation.bat"
