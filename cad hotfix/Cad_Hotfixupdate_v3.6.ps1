<#
.SYNOPSIS
    ProPhoenix CAD - PD (Production / Installation Team).
    
    FINAL CORRECTIONS:
    1. CRASH FIX: Removed the "$plainMsg = $Msg -replace..." line entirely to fix the "Invalid regular expression pattern ?" error.
    2. ORDER: App Manager Update -> Step 1.
    3. EXCLUSIONS: Always skips "Hub" and "Gateway".
    4. SECONDARY BAT: Creates "Instance Update(BS).bat" (overwrites every time) and only stops/starts services.
#>
...
$GeneratorLogic = {
    ...
    # 1. CLEANUP SCRIPT (Session Data Only)
...
function Log-Msg {
    ...
    if (`$cleanLogFile -and -not `$NoFile) {
        # Directly output to file. No replacement regex needed. Pure ASCII text.
        ...
        if (Test-Path `$instancesPath) {
            ...
            foreach (`$env in `$instanceEnvs) {
                ...
                if (`$status -eq "Completed") {
                    ...
                    `$notCompletedCount++
                    Log-Msg ("[FAIL] " + `$app.Name + " -> " + `$env.Name + " : Not Completed") "Red"
                }
            }
        }
    }
...
"@
    ...
    Write-Host "   ProPhoenix CAD - PD (Production / Installation Team)  " -ForegroundColor Yellow
...
"@
        ...
        Write-Host " [SUCCESS] Generated Main Bat: $OutputBatFile" -ForegroundColor Green


        # ======================================================================
        #  SECONDARY BATCH FILE: INSTANCE UPDATE & VERIFICATION ONLY
        # ======================================================================
        $SpecialBatDir = "C:\pnxtemp\Phoenix Installation Master\Phoenix Installation Master"
        
        if (-not (Test-Path $SpecialBatDir)) { 
            try { New-Item -ItemType Directory -Path $SpecialBatDir -Force | Out-Null } catch {} 
        }

        $InstanceBatchContent = @"
$InstallDrive
cd ../../../../
cd "$AppMgrFolder"
@echo off
...
set "timestamp=%DATE:/=-%_%TIME::=-%"
set "timestamp=%timestamp: =0%"
set LOGFILE="%~dp0InstanceUpdateOnly_Log_%timestamp%.txt"

echo ===================================================
echo   PHOENIX INSTANCE UPDATE AND VERIFICATION ONLY
echo ===================================================
echo [LOG] Execution Started... > %LOGFILE%
...
echo ===================================================
echo [LOG] Verifying DLLs... >> %LOGFILE%
%PSExe% -ExecutionPolicy Bypass -File "$PnxTempPath\InstanceVerification.ps1"
...
"@
        ...
        # --- SAVE SECONDARY BATCH FILE (OVERWRITES EVERY TIME) ---
        $InstFinalName = "Instance Update(Batch).bat"
        $InstOutputBatFile = Join-Path -Path $SpecialBatDir -ChildPath $InstFinalName
        Set-Content -Path $InstOutputBatFile -Value $InstanceBatchContent -Encoding ASCII
        Write-Host " [SUCCESS] Generated Instance-Only Bat: $InstOutputBatFile" -ForegroundColor Green
...
if ($RunRemote) {
    ...
    if ($LocalBatFiles.Count -gt 0) {
        if ((Read-Host "`nRun Automation now? (Y/N)") -eq "Y") {
            # Start processes with Admin Rights per rules
