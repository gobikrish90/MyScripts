...
.SYNOPSIS
    ...
    FIXES APPLIED (Previous + New):
    1. PATH QUOTE FIX: Added .Trim('"') to $env:LOGFILE to fix "Drive '"D' does not exist" error.
    2. FILE LOCK FIX: Added try/catch to Log Cleaner to skip locked files instead of crashing.
    3. COMPATIBILITY: Uses 'Out-File' compatible with older PowerShell versions.
#>
...
$GeneratorLogic = {
    ...
    # 1. CLEANUP SCRIPT (FIXED LOCKED FILE ERROR)
...
Write-Host "[INFO] Clearing Log Files..."
foreach (`$basePath in `$proPhoenixBasePaths) {
    Get-ChildItem -Path `$basePath -Directory -Recurse | Where-Object { Test-Path "`$(`$_.FullName)\_Instances" } | ForEach-Object {
        `$instRoot = "`$(`$_.FullName)\_Instances"
        Get-ChildItem -Path `$instRoot -Directory | ForEach-Object {
            `$logPath = "`$(`$_.FullName)\PnxLog"
            `$oldPath = "`$logPath\old"
            if (Test-Path `$logPath) {
                if (!(Test-Path `$oldPath)) { New-Item -ItemType Directory -Path `$oldPath | Out-Null }
                ...
                # MOVE CURRENT LOGS (WITH ERROR HANDLING)
                Get-ChildItem -Path `$logPath -File | Where-Object { `$_.FullName -notlike "*\old*" } | ForEach-Object {
                    try {
                        Move-Item -Path `$_.FullName -Destination `$oldPath -Force -ErrorAction Stop
                        Write-Host " [CLEANED] Log: `$(`$_.Name)"
                    } catch {
                        Write-Host " [SKIP] Locked File: `$(`$_.Name)" -ForegroundColor DarkGray
                    }
                }
            }
        }
    ...
    if (Test-Path `$cadServerPath) {
        ...
        foreach (`$inst in `$cadInstances) {
            ...
            if (Test-Path `$sessionDataPath) {
                Write-Host " [CHECKING] `$sessionDataPath"
                Get-ChildItem -Path `$sessionDataPath -File | ForEach-Object {
                    try { Remove-Item -Path `$_.FullName -Force -ErrorAction Stop } catch { Write-Host " [SKIP] Locked: `$(`$_.Name)" -ForegroundColor DarkGray }
                }
            }
        }
    }
...
"@
    ...
    # 2. VERIFICATION SCRIPT (FIXED QUOTE/DRIVE ERROR)
...
function Log-Msg {
    ...
    if (`$env:LOGFILE) { 
        # FIX: Trim quotes from path to prevent "Drive not found" error
        `$CleanLogPath = `$env:LOGFILE.Trim('"')
        if (Test-Path (Split-Path `$CleanLogPath)) {
            `$Msg | Out-File -FilePath `$CleanLogPath -Append -Encoding ASCII -ErrorAction SilentlyContinue 
        }
    }
}
    ...
    foreach (`$app in `$appFolders) {
        ...
        if (Test-Path `$instancesPath) {
            ...
            foreach (`$env in `$instanceEnvs) {
                ...
                else { `$notCompletedCount++; Log-Msg "[FAIL] `$(`$app.Name) -> `$(`$env.Name)" "Red" }
            }
        }
    }
}
...
echo ===================================================
CHOICE /C YN /M "Do you want to install STAGE Applications now?"
IF ERRORLEVEL 2 GOTO SkipStage
IF ERRORLEVEL 1 GOTO RunStage
...
"@
        ...
        $FinalName = "${CurrentHostName}_PROD_Install_$(Get-Date -Format 'HHmmss').bat"
