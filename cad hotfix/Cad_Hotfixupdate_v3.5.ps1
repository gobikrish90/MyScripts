<#
.SYNOPSIS
    ProPhoenix Hotfix Automation (CAD - PD).
...
Write-Host "   ProPhoenix Hotfix Automation (CAD - PD)  " -ForegroundColor Cyan
Write-Host "==============================================" -ForegroundColor Cyan
...
function Log-Msg {
    ...
    if (`$cleanLogFile -and -not `$NoFile) {
        `$plainMsg = `$Msg -replace '✅', '[OK]' -replace '❌', '[FAIL]' -replace '→', '->'
        `$plainMsg | Out-File -FilePath `$cleanLogFile -Append -Encoding ASCII -ErrorAction SilentlyContinue
    }
}
...
"@
    ...
    Write-Host "   ProPhoenix Hotfix Automation (CAD - PD)  " -ForegroundColor Yellow
