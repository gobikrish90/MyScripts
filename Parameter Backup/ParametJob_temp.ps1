...
$sharesToManage = @{
    ...
    "Bolo" = "CAD Server\_Instances\Live\Bolo" # Default fallback for jobs, maps dynamically if needed
}
...
$paramValues = @{
    ...
    222  = "\\$hostName\Watch"   # NEW Parameter requested
    231  = "1"
...
try {
    ...
    # ==========================================
    # 6. Enterprise Parameter UPSERT Execution
    # ==========================================
...
"@
        ...
        try {
            ...
            if ($actionResult -eq 'Updated') { Write-Host " [Updated]  ParamID $paramID -> $safeValue" -ForegroundColor Green; $totalUpdated++ } 
            elseif ($actionResult -eq 'Inserted') { Write-Host " [Inserted] ParamID $paramID -> $safeValue" -ForegroundColor Yellow; $totalInserted++ }
        } catch { Write-Host " [Error] Failed on ParamID $($paramID): $_" -ForegroundColor Red }
    }
    Write-Host "`nParameter Summary: $totalUpdated updated, $totalInserted inserted." -ForegroundColor Cyan
    ...
    function Setup-KPIJob($jobName, $jobInstance, $folder1, $folder2) {
    ...
    if ($productChoice -eq "1" -or $productChoice -eq "") {
        # POLICE JOBS
        Setup-KPIJob -jobName "WDAAppDataExporter" -jobInstance $selectedCadInstance -folder1 "\\$hostName\Fire Response CAD Webservice"
