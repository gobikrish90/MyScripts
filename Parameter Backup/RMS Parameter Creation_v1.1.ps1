...
Write-Host "  ProPhoenix Enterprise Setup & DB Parameter Updater" -ForegroundColor Cyan
Write-Host "=======================================================" -ForegroundColor Cyan
...
# ==========================================
# 6. Enterprise UPSERT (Update or Insert) Execution
# ==========================================
Write-Host "`nExecuting Parameters in Database [$selectedDb]..." -ForegroundColor Cyan
...
try {
    ...
    foreach ($paramID in $paramValues.Keys) {
        $rawValue = $paramValues[$paramID]
        $safeValue = $rawValue.Replace("'", "''")
        
        # Enterprise-grade UPSERT block with dynamic JurisID selection
        $upsertQuery = @"
        SET NOCOUNT ON;
        ...
        BEGIN
            UPDATE Parameter 
            SET ParamValue = @tVal 
            WHERE CAST(ParamID AS VARCHAR(50)) = CAST(@tID AS VARCHAR(50));
        ...
        BEGIN
            ...
            -- Dynamically grab a valid JurisID from an existing system parameter
            ...
            INSERT INTO Parameter (SLNo, ParamID, ParamValue, JurisID) 
            VALUES (@nextSLNo, @tID, @tVal, @sysJurisID);
...
"@
        ...
        try {
            ...
            if ($actionResult -eq 'Updated') {
                Write-Host " [Updated]  ParamID $paramID -> $rawValue" -ForegroundColor Green
                $totalUpdated++
            } elseif ($actionResult -eq 'Inserted') {
                Write-Host " [Inserted] ParamID $paramID -> $rawValue" -ForegroundColor Yellow
                $totalInserted++
            }
        } catch {
            Write-Host " [Error] Failed on ParamID $($paramID): $_" -ForegroundColor Red
        }
    }
    
    Write-Host "`nExecution Summary: $totalUpdated updated, $totalInserted new records inserted." -ForegroundColor Cyan
