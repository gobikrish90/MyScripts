...
# --- HARDCODED EXCLUSIONS ---
        $ExcludedIDs = @("PhoenixHub", "PhoenixGateway", "PaymentGateway")
...
"@
    ...
    # Copy the verification script into this secondary folder so it can execute locally
    ...
    $SecondaryBatFile = Join-Path -Path $SecondaryBatDir -ChildPath "Instance Update(Batch).bat"
...
echo.
echo ===================================================
echo   INSTANCE UPDATE COMPLETE - PAUSED
echo ===================================================
echo Please review the verification status above.
echo Press any key to close the screen...
pause >nul
...
"@
    ...
    try {
        # Using Set-Content to overwrite every time as requested (no counter loop applied here)
...
if ($OutputBatFile -and (Test-Path $OutputBatFile)) {
    Write-Host ""
    $UserResponse = Read-Host "Do you want to execute the PRIMARY generated script now? (Y/N)"
