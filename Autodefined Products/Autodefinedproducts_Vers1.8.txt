<#
.SYNOPSIS
    ProPhoenix Full Maintenance Generator (CRASH FIX).
    1. Replaces invalid PowerShell syntax (*>&1) with Batch syntax (2>&1).
    2. Fixes the "Script closes immediately" issue.
    3. Runs Verification and Service Start smoothly.
#>
...
foreach ($FoundPath in $FoundFiles) {
    ...
    if ($UpdateArgs.Count -gt 1) {
        # FIX: Replaced *>&1 with 2>&1 (CMD Standard Error Redirection)
        $UpdateCmd = "::Script - Updating Windows Service Instance`r`n%AppMgrExePath%\PnxAppMgr.exe $UpdateLine | %PSExe% -Command `"`$input | ForEach-Object { Write-Host `$_; `$_ | Out-File -FilePath '%LOGFILE%' -Append -Encoding ASCII }`" 2>&1`r`n"
    }

    # --- BATCH FILE CONTENT ---
    # FIX: Replaced all *>&1 occurrences with 2>&1 (The Root Cause of the crash).
    $BatchContent = @"
$CdBlock
@echo off
...
%PSExe% -Command "Get-Service -Name 'Phoenix*' | Where-Object { `$_.Status -eq 'Stopped' } | ForEach-Object { Start-Service -Name `$_.Name; Write-Host 'Started ' `$_.Name }" | %PSExe% -Command "`$input | ForEach-Object { Write-Host `$_; `$_ | Out-File -FilePath '%LOGFILE%' -Append -Encoding ASCII }" 2>&1
