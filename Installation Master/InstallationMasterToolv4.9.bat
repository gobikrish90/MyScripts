<# : Polyglot Header - Executed by CMD
@echo off
title Prophoenix Dashboard Launcher
color 0A

:: ==============================================================================
::  1. BATCH ADMIN CHECK (Fixed for spaces in file paths)
:: ==============================================================================
net session >nul 2>&1
if %errorLevel% NEQ 0 (
    echo =====================================================
...
:: ==============================================================================
::  2. PASS PATHS TO POWERSHELL & EXECUTE WITH ERROR CATCHING
:: ==============================================================================
set "BAT_PATH=%~f0"
set "BAT_DIR=%~dp0"

echo Loading PowerShell environment...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$path = [Environment]::GetEnvironmentVariable('BAT_PATH'); try { Invoke-Expression (Get-Content -LiteralPath $path -Raw) } catch { Write-Host 'CRITICAL POWERSHELL ERROR: ' $_.Exception.Message -ForegroundColor Red; Read-Host 'Press ENTER to close' }"

echo.
echo Batch execution finished. If the dashboard didn't open, look for errors above.
pause
exit /b
#>
...
# ==============================================================================
#  1. ROBUST PATH DETECTION (Adjusted for Polyglot)
# ==============================================================================
try {
    $ScriptPath = [Environment]::GetEnvironmentVariable('BAT_DIR')
    if ($ScriptPath -match '\\$') { $ScriptPath = $ScriptPath.TrimEnd('\') }
...
try {
    ...
    function Get-FriendlyName($fileName) {
        ...
        $cleanName = $cleanName -replace "-v\d+\.\d+", ""  
        $cleanName = $cleanName -replace "v\d+\.\d+", ""   
