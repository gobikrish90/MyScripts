<#
.SYNOPSIS
    Prophoenix Installation Dashboard v8.7
    - UI FIX: Set Execution Logs to dynamically Fill the panel (fixes the small black box issue).
    - UI FIX: Reduced card height/spacing so all scripts fit on one screen without a scrollbar.
    - UI FIX: Increased label width to give long script names (like Instance Update Batch) extra space.
    - UI FIX: Changed Splash Screen title to "Installation Hotfix Dashboard".
    - CORE: Retains ADSI Telemetry, Glass Theme, Guide, and Layout.
#>
...
Write-Host "[INIT] Booting Prophoenix Installation Dashboard v8.7..." -ForegroundColor Cyan
...
function Refresh-List {
    ...
    # START HEIGHT ADJUSTED AND PADDING DECREASED TO FIT WITHOUT SCROLLBAR
    [int]$Y = 10 
    ...
    foreach ($item in $SortedList) {
        ...
        $card.Size = New-Object System.Drawing.Size(890, 36) # HEIGHT REDUCED TO FIT
        ...
        $l.Font = $fontRowText
        $l.Left = 20; $l.Top = 7; $l.Width = 700 # WIDTH EXPANDED TO PREVENT TEXT CUT OFF
        ...
        $Y += 40 # SPACING REDUCED TO FIT WITHOUT SCROLLBAR
    }
}
...
$script:pnlConsole.Size = New-Object System.Drawing.Size(910, 600) # Explicitly set size before rounding
$script:pnlConsole.Dock = [System.Windows.Forms.DockStyle]::Fill
$script:pnlConsole.BackColor = Get-Color "Terminal"
$script:pnlConsole.Padding = New-Object System.Windows.Forms.Padding(15)
$script:pnlConsole.Visible = $false
Set-RoundedCorner $script:pnlConsole 10
$script:pnlToolsWrapper.Controls.Add($script:pnlConsole)

$script:TermConsole = New-Object System.Windows.Forms.RichTextBox
$script:TermConsole.Dock = [System.Windows.Forms.DockStyle]::Fill # Tells textbox to fill the entire space inside padding
...
# --- EXECUTE ON STARTUP (AUTO-DOWNLOAD & EXTRACT NEW SCRIPTS) ---
$script:form.Add_Shown({ 
    ...
    # ---------------------------------------------------------
    # PLACE YOUR GITHUB DIRECT DOWNLOAD ZIP URL HERE:
    $Url_GitHub = "https://github.com/YourUsername/YourRepo/archive/refs/heads/main.zip"
    # ---------------------------------------------------------
    
    $TempZip = "C:\PnxTemp\LatestMaster.zip"
    ...
    # 2. Download the new zip automatically
    Write-Terminal "Downloading latest payload from GitHub..." "White"
    Update-Status "Downloading latest scripts..."
    [System.Windows.Forms.Application]::DoEvents()
    
    try {
        [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
        Invoke-WebRequest -Uri $Url_GitHub -OutFile $TempZip -UseBasicParsing
        Write-Terminal "Download completed successfully." "Lime"
    } catch {
        Write-Terminal "Failed to download from GitHub: $_" "Red"
        Update-Status "Download Failed." $true
        return
    }
    
    # 3. Extract the new zip (works regardless of the zip's name)
    Write-Terminal "Extracting new payload..." "White"
    ...
    # GitHub ZIPs extract into a subfolder (e.g., RepoName-main). This finds that nested folder automatically.
    $NestedDir = Get-ChildItem -Path $InstallBase -Directory | Select-Object -First 1
