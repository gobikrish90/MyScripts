<#
.SYNOPSIS
    Prophoenix Installation Dashboardv5.8
#>
...
Write-Host "[INIT] Prophoenix Installation Dashboard" -ForegroundColor Cyan
...
try {
    ...
    $script:Font_ItemText  = New-Object System.Drawing.Font("Segoe UI", 9, [System.Drawing.FontStyle]::Bold)
    ...
    function Set-FolderPermissions($path) {
        Write-Terminal "Applying advanced ACLs to Directory ($path)..." "Yellow"
        [System.Windows.Forms.Application]::DoEvents()
        try { 
     ...
     function Launch-File($path, $friendlyName) {
        ...
        try {
            ...
            elseif ($path -match "Minimal Downtime") { 
                # Specific Minimal Downtime scripts open directly in ISE
                Start-Process "powershell_ise.exe" -ArgumentList "-NoProfile -File `"$path`"" -Verb RunAs -WorkingDirectory $workDir 
            } 
            else { 
                # All other .ps1 scripts open in a separate, persistent PowerShell window
    ...
    function Refresh-List {
        ...
        foreach ($item in $SortedList) {
            ...
            $card.Size = New-Object System.Drawing.Size(860, 26)
            ...
            $b.Text = "LAUNCH"
            $b.Left = 750; $b.Top = 2
            $b.Size = New-Object System.Drawing.Size(90, 22) 
    ...
    function Search-Master {
        ...
        try {
            ...
            if ($FoundZip) {
                ...
                # Explicitly apply permissions post-extraction
    ...
    # BUTTONS SHIFTED UP TO REPLACE LOGO SPACE
    Add-SideHeader "General" 30 "HeaderRed"
    ...
    $lblTitle = New-Object System.Windows.Forms.Label; $lblTitle.Text="ProPhoenix Installation Dashboard"; $lblTitle.Font=$script:Font_MainTitle; $lblTitle.Left=95; $lblTitle.Top=30; $lblTitle.AutoSize=$true; $lblTitle.ForeColor=Get-Color "TextWhite"; $script:pnlHead.Controls.Add($lblTitle)
    ...
    $script:btnTabScripts = New-Object System.Windows.Forms.Button; $script:btnTabScripts.Text="Available Scripts"; $script:btnTabScripts.Font=$script:Font_SideHead; $script:btnTabScripts.Size=New-Object System.Drawing.Size(760, 40); $script:btnTabScripts.Left=0; $script:btnTabScripts.BackColor=Get-Color "CardBg"; $script:btnTabScripts.ForeColor=Get-Color "TextWhite"; $script:btnTabScripts.FlatStyle="Flat"; $script:btnTabScripts.FlatAppearance.BorderSize=1; $script:btnTabScripts.FlatAppearance.BorderColor=Get-Color "TextWhite"; $script:pnlTabs.Controls.Add($script:btnTabScripts)
    ...
    # --- TERMINAL TOGGLE BUTTON (RENAMED & RE-SIZED) ---
    $script:btnToggleLog = New-Object System.Windows.Forms.Button
    $script:btnToggleLog.Text = "▲ Execution Logs"
    $script:btnToggleLog.Size = New-Object System.Drawing.Size(150, 35)
    $script:btnToggleLog.Anchor = [System.Windows.Forms.AnchorStyles]::Bottom -bor [System.Windows.Forms.AnchorStyles]::Right
    $script:btnToggleLog.Location = New-Object System.Drawing.Point(740, 550)
    $script:btnToggleLog.BackColor = Get-Color "BtnBlue"
    ...
    $ToggleAction = {
        ...
        if ($script:pnlConsole.Visible) {
            ...
            $script:btnToggleLog.Text = "▼ Hide Logs"
            $script:btnToggleLog.BackColor = Get-Color "BtnRed"
        }
    ...
    $script:form.Add_Shown({ 
        ...
        if (Test-Path $InstallBase) {
            # VISIBLE PERMISSION AND UNBLOCK LOGIC ON STARTUP
            if (-not $script:pnlConsole.Visible) { $script:btnToggleLog.PerformClick() }
            Write-Terminal "Payload detected at startup. Verifying System Security..." "Cyan"
            [System.Windows.Forms.Application]::DoEvents()
            ...
            # Hide logs automatically after successful boot load
            Start-Sleep -Seconds 1
            if ($script:pnlConsole.Visible) { $script:btnToggleLog.PerformClick() }
