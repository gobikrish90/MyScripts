<#
.SYNOPSIS
    Prophoenix Installation Dashboard (v5.0 - Installation Team)
...
$form.Text = "Prophoenix Installation Dashboard v52.0"
...
function Get-DownloadsPath {
    # Method 1: Registry (Most accurate for redirected folders/OneDrive)
    try {
...
function Search-Master {
    ...
    # Priority Paths: REAL DOWNLOADS FIRST
    $SearchDirs = @(
        $RealDownloads,
    ...
    foreach ($dir in $SearchDirs) {
        if ($dir -and (Test-Path $dir)) {
            # Find ANY file matching pattern, Sort by NEWEST
    ...
    if ($FoundZip) {
        ...
        # Copy to C:\PnxTemp first to avoid OneDrive path issues
        try { Copy-Item -Path $FoundZip -Destination $LocalZip -Force } 
        catch { 
            [System.Windows.Forms.MessageBox]::Show("Copy Failed. Source: $FoundZip`nError: $_", "Copy Error")
            $form.Cursor = [System.Windows.Forms.Cursors]::Default; return 
        }

        # Extract
        try { Expand-Archive -Path $LocalZip -DestinationPath $InstallBase -Force } 
        catch { 
            [System.Windows.Forms.MessageBox]::Show("Extraction Failed. Error: $_", "Error")
            $form.Cursor = [System.Windows.Forms.Cursors]::Default; return 
        }
