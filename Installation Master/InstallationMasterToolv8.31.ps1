...
function global:Execute-ScheduledJob($job) {
    global:Write-Terminal ">>> SCHEDULE TRIGGERED: $($job.FriendlyName)" "Cyan" "ALL" ">>> SCHEDULE TRIGGERED: $($job.FriendlyName)"
...
cd /d "$remoteWorkDir"
powershell.exe -ExecutionPolicy Bypass -File "$remotePath" < "$ansPath" >> "$remoteLog" 2>&1
...
for /f "tokens=*" %%F in ('dir /b /o:-d /a:-d "$remoteWorkDir\*.bat" ^| findstr /v /i "wrapper"') do (
    ...
    cmd.exe /c "$remoteWorkDir\%%F" < "$ansPath" >> "$remoteLog" 2>&1
    goto :BatchDone
)
:BatchDone
...
"@
        ...
        try {
            Invoke-Command -ComputerName $t.HostName -Credential $t.Credential -ScriptBlock $sbCreate -ArgumentList $wrapperPath, $wrapperContent, $taskName, $remoteLog, $ansPath, $ansContent
...
function global:Launch-RemoteBat($HostName, $Cred, $BatPath, $UserInput) {
    global:Write-Terminal ">> DASHBOARD INTERCEPT: User confirmed. Executing primary batch file..." "Yellow" $HostName ">> DASHBOARD INTERCEPT: User confirmed. Executing primary batch file..."
    ...
    $sb = {
        ...
        cmd.exe /c "timeout /t 2 >nul & `"$bPath`" < `"$tmpFile`"" 2>&1 | ForEach-Object { Write-Output "BATCH> $_" }
...
function global:Launch-File($path, $friendlyName) {
    ...
    if ($needsPrePrompt) {
        ...
        $lblInfo.Text = "Executing on: $targetName`nScript: $friendlyName`n`nIMPORTANT: If the script asks for input, enter it below. For MULTIPLE prompts, separate with commas (e.g., Y,1,N)."
...
function global:Launch-File($path, $friendlyName) {
    ...
    # REQUIRED: Feed 'N' to bypass the initial internal execution loop of the ps1 files
    $userInput = "N"

    $needsPrePrompt = ($friendlyName -eq "Test/Demo Hotfix")
        ...
        if ($global:ActiveJobs.Count -gt 0) {
            ...
            foreach ($aj in $global:ActiveJobs) {
                ...
                if ($results) {
                    foreach ($res in $results) {
                        if ($res -match "^\[DASHBOARD_LIVE_PROMPT\]\|(.+)$") {
                            ...
                            global:Write-Terminal ">> SCRIPT PAUSED: Batch file generated ($batPath). Please use the Live Input Bar below to continue." "Yellow" $aj.Target ">> SCRIPT PAUSED: Batch file generated. Please use the Live Input Bar below to continue."
                ...
                if ($aj.Job.State -in 'Completed','Failed','Stopped') {
                    if ($aj.Name -ne "Batch Execution") {
                        global:Write-Terminal "[$($aj.Target)] [JOB: $($aj.Name)] Script block completed." "Lime" $aj.Target "[JOB: $($aj.Name)] Script block completed."
                    } else {
