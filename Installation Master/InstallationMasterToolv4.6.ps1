<#
.SYNOPSIS
    Prophoenix Installation Dashboard (v71.0 - Syntax Fix)
    - CRITICAL FIX: Resolved "Variable reference is not valid" error by using ${friendlyName} syntax.
    - CORE: Retains Deep Search, Logs, Auto-Repair, and Background Logic.
#>
...
$form.Text = "Prophoenix Installation Dashboard v71.0"
...
function Launch-File($path, $friendlyName) {
    ...
    try {
        ...
        # FIX: Corrected Syntax for Variable Interpolation
...
function Search-Master {
    ...
    if (-not $FoundZip) {
        $msg = "Deep Search failed.`nPlease select 'Phoenix Installation Master.zip' manually."
        ...
        if ($dlg.ShowDialog() -eq "OK") {
            ...
            Write-GlobalLog "Manual Selection: $FoundZip"
        } else {
