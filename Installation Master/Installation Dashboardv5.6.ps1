<#
.SYNOPSIS
    ProPhoenix Enterprise Deployment Dashboard (v118.0)
    - UI FIX: List buttons changed to "LAUNCH".
    - LOGIC FIX: Exact Sort Order injected.
    - LOGIC FIX: Restored previous hybrid launch model (Separate PS console + ISE for specific scripts).
    - CORE: Modern dark theme, Telemetry, Zero-scroll watermark, and ACL Permissions.
#>

Write-Host "[INIT] Booting ProPhoenix Enterprise Command Center v118.0..." -ForegroundColor Cyan
...
try {
    ...
    $script:Font_ItemText  = New-Object System.Drawing.Font("Segoe UI", 10, [System.Drawing.FontStyle]::Bold)
    ...
    function Test-SftpConnection {
        $script:form.Cursor = [System.Windows.Forms.Cursors]::WaitCursor
        Update-Status "Testing SFTP Connection..."
        if (-not $script:pnlConsole.Visible -and $script:btnToggleLog) { $script:btnToggleLog.PerformClick() }
        ...
        try {
            ...
            if ($result.TcpTestSucceeded) {
                ...
                Update-Status "SFTP Connection Successful."
            } else { 
                Write-Terminal "STATUS: FAILED (TCP Port 25544 is CLOSED or BLOCKED)" "Red" 
                Update-Status "SFTP Connection Failed." $true
            }
        } catch { Write-Terminal "ERROR: Test failed." "Red" } 
        finally { $script:form.Cursor = [System.Windows.Forms.Cursors]::Default }
    }

    # ==========================================================================
    #  5. EXACT SORT ORDER & PREVIOUS LAUNCH LOGIC
    # ==========================================================================
    ...
    # --- RESTORED PREVIOUS EXECUTION LOGIC (ISE / SEPARATE CONSOLE) ---
    ...
    function Refresh-List {
        ...
        if (-not $script:ToolsLoaded) {
            ...
            $lbl.Text="No Scripts Detected. Please click 'Reload Scripts'."
            $lbl.AutoSize=$true
        ...
        foreach ($item in $SortedList) {
            ...
            $card.Size = New-Object System.Drawing.Size(860, 45)
            ...
            $l.Text = $item.Friendly # Proper name string mapped from sorting logic
            $l.Font = $script:Font_ItemText
            $l.Left = 15; $l.Top = 12; $l.Width = 650
    ...
    function Search-Master {
        $script:form.Cursor = [System.Windows.Forms.Cursors]::WaitCursor
        Update-Status "Reloading Scripts..."
        if (-not $script:pnlConsole.Visible -and $script:btnToggleLog) { $script:btnToggleLog.PerformClick() }
        ...
        try {
            ...
            if ($FoundZip) {
                ...
                Refresh-List
                Update-Status "Scripts Loaded Successfully."
            }
        } finally {
            $script:form.Cursor = [System.Windows.Forms.Cursors]::Default
        }
    ...
    $script:form.Text = "New Installation Dashboard Panel"
    ...
    Add-SidePillButton "Reload Scripts" 210 "BtnRed" "BtnRedHover" { Search-Master }
    ...
    Add-SidePillButton "Check SFTP Connection" 435 "BtnBlue" "BtnBlueHover" { Test-SftpConnection }
    ...
    $lblTitle = New-Object System.Windows.Forms.Label; $lblTitle.Text="New Installation Dashboard Panel"; $lblTitle.Font=$script:Font_MainTitle; $lblTitle.Left=260; $lblTitle.Top=30; $lblTitle.AutoSize=$true; $lblTitle.ForeColor=Get-Color "TextWhite"; $script:pnlHead.Controls.Add($lblTitle)
    $lblSubTitle = New-Object System.Windows.Forms.Label; $lblSubTitle.Text="Extract and execute your scripts seamlessly."; $lblSubTitle.Font=$script:Font_SubTitle; $lblSubTitle.Left=265; $lblSubTitle.Top=65; $lblSubTitle.AutoSize=$true; $lblSubTitle.ForeColor=Get-Color "TextGrey"; $script:pnlHead.Controls.Add($lblSubTitle)

    # --- TELEMETRY CARDS (TOP RIGHT) ---
    ...
    $script:btnTabLogs = New-Object System.Windows.Forms.Button; $script:btnTabLogs.Text="Execution Logs"; $script:btnTabLogs.Font=$script:Font_SideHead; $script:btnTabLogs.Size=New-Object System.Drawing.Size(430, 40); $script:btnTabLogs.Left=440; $script:btnTabLogs.BackColor=Get-Color "BgMain"; $script:btnTabLogs.ForeColor=Get-Color "TextGrey"; $script:btnTabLogs.FlatStyle="Flat"; $script:btnTabLogs.FlatAppearance.BorderSize=0; $script:pnlTabs.Controls.Add($script:btnTabLogs)
