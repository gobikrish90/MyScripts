<#
.SYNOPSIS
    Installation Master Dashboard v8.49 (Enterprise Deep-Hunt Framework)
 #>
...
Write-Host "[INIT] Booting Enterprise Dashboard v8.49..." -ForegroundColor Cyan
...
function global:Check-Updates {
    global:Show-ExecutionLogs
    global:Write-Terminal "Checking GitHub repository for latest payload & tools..." "Yellow" "ALL"
    ...
    try {
        ...
        $localToolVerFile = "C:\PnxTemp\CurrentDashboardVersion.txt"
        $currentToolVer = if (Test-Path $localToolVerFile) { Get-Content $localToolVerFile } else { "" }

        if ($latestTool -and $latestTool.name -ne $currentToolVer) {
            global:Write-Terminal "Found new Master Tool update on GitHub: $($latestTool.name)" "Cyan" "ALL"
            $TempToolZip = "C:\PnxTemp\$($latestTool.name)"
            Invoke-WebRequest -Uri $latestTool.download_url -OutFile $TempToolZip -Headers $headers -UseBasicParsing
            if (Test-Path $TempToolZip) {
                ...
                Remove-Item $TempToolZip -Force -ErrorAction SilentlyContinue
                global:Unblock-ExtractedFiles $extractDir $true
                
                $newExe = Get-ChildItem $extractDir -Filter "*.exe" -Recurse -ErrorAction SilentlyContinue | Select-Object -First 1
                $newPs1 = Get-ChildItem $extractDir -Filter "*.ps1" -Recurse -ErrorAction SilentlyContinue | Select-Object -First 1
                
                if ($newExe -or $newPs1) {
                    Set-Content -Path $localToolVerFile -Value $latestTool.name -Force
                    $toolUpdated = $true
                    
                    if ($newExe) {
                        global:Write-Terminal "Launching updated Master Tool executable..." "Lime" "ALL"
                        Start-Process -FilePath $newExe.FullName -WorkingDirectory $newExe.DirectoryName -Verb RunAs
                    } elseif ($newPs1) {
                        global:Write-Terminal "Launching updated Master Tool script..." "Lime" "ALL"
                        Start-Process -FilePath "powershell.exe" -ArgumentList "-NoProfile -ExecutionPolicy Bypass -File `"$($newPs1.FullName)`"" -WorkingDirectory $newPs1.DirectoryName -Verb RunAs
                    }
                    
                    global:Update-Status "Update applied. Restarting..."
                    Start-Sleep -Seconds 2
                    $script:form.Close()
                    return
                }
        ...
        $currentPayloadVer = if (Test-Path $localPayloadVerFile) { Get-Content $localPayloadVerFile } else { "" }

        if ($latestPayload -and $latestPayload.name -ne $currentPayloadVer) {
            ...
            if (Test-Path $TempZip) {
                ...
                global:Write-Terminal "Payload updated successfully." "Lime" "ALL"
                $updated = $true
            }
        }
        ...
        } elseif (-not $toolUpdated) {
            global:Write-Terminal "No new update available." "Lime" "ALL"
            global:Update-Status "No new update available."
        }
    } catch {
        global:Write-Terminal "No new update available (or API limit reached)." "Yellow" "ALL"
        global:Update-Status "No new update available."
    }
}
...
function global:Switch-NewServerDashboard {
    global:Show-ExecutionLogs
    global:Write-Terminal "Searching for New Server Dashboard package..." "Cyan" "ALL"
    
    $searchLocations = @("C:\PnxTemp\Phoenix Installation Master", "C:\PnxTemp", $script:CurrentToolPath)
    $zipFile = $null

    foreach ($loc in $searchLocations) {
        if (Test-Path $loc) {
            $found = Get-ChildItem -Path $loc -Filter "*.zip" -Recurse -ErrorAction SilentlyContinue | 
                     Where-Object { $_.Name -match "(?i)New\s*Server\s*Dashboard" -or $_.Name -match "(?i)New\s*Server\s*Installation" } | 
...
Add-SidebarButton $pnlGroupSys "Switch to New Server Dashboard" 130 $colorBtnSys { global:Switch-NewServerDashboard }
...
if ($null -ne $script:form) {
    $script:form.Add_Shown({ 
        try {
            ...
            $Url_GitHub = "https://github.com/gobikrish90/MyScripts/raw/main/Phoenix%20Installation%20Master_20260731_1545.zip"
            $Url_GDrive = "https://drive.google.com/uc?export=download&id=10RxuJaWwqR1S6lbkjL0-_AXddCwOARYI"
