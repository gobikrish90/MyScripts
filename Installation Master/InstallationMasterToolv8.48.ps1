<#
.SYNOPSIS
    Installation Master Dashboard v8.48 (Enterprise Deep-Hunt Framework)
 #>
...
Write-Host "[INIT] Booting Enterprise Dashboard v8.48..." -ForegroundColor Cyan
        ...
        if ($latestTool) {
            ...
            if (Test-Path $TempToolZip) {
                ...
                if ($newExe) {
                    ...
                    Start-Process -FilePath $newExe.FullName -Verb RunAs
                    Start-Sleep -Seconds 2
                    $script:form.Close()
                    return
                } elseif ($newPs1) {
                    global:Write-Terminal "Launching updated Master Tool script..." "Lime" "ALL"
                    Start-Process -FilePath "powershell.exe" -ArgumentList "-NoProfile -ExecutionPolicy Bypass -File `"$($newPs1.FullName)`"" -Verb RunAs
                    Start-Sleep -Seconds 2
                    $script:form.Close()
                    return
                }
...
function global:Switch-NewServerDashboard {
    ...
    if ($zipFile -and (Test-Path $zipFile)) {
        ...
        try {
            ...
            # Automatically find the DB Script matching the image_668324.png reference (*DB Script*.ps1)
...
if ($null -ne $script:form) {
    $script:form.Add_Shown({ 
        try {
            ...
            $Url_GitHub = "https://github.com/gobikrish90/MyScripts/raw/main/Phoenix%20Installation%20Master_20260731_1454.zip"
            $Url_GDrive = "https://drive.google.com/uc?export=download&id=10RxuJaWwqR1S6lbkjL0-_AXddCwOARYI"
