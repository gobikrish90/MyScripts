<#
.SYNOPSIS
    Installation Master Tool v8.34
#>
...
Write-Host "[INIT] Booting Installation Master Tool v8.34..." -ForegroundColor Cyan
...
function global:Start-ScreenRecording {
    param(
    ...
    try {
        $ffmpegPath = Join-Path $script:CurrentToolPath "ffmpeg.exe"

        if (-not (Test-Path $ffmpegPath)) {
            global:Write-Terminal "FFmpeg not found at $ffmpegPath" "Red" $HostName
            return $null
        }

        $recordRoot = "C:\PnxTemp\RemotePayloads\Recordings\$HostName"

        if (-not (Test-Path $recordRoot)) {
        ...
        $videoFile = Join-Path $recordRoot "$safeName`_$timestamp.mp4"
        ...
        $global:RecordingSessions[$HostName] = $session

        global:Write-Terminal "Recording started: $videoFile" "Cyan" $HostName

        return $session
    }
    catch {
        global:Write-Terminal "Recording start failed: $_" "Red" $HostName
        return $null
    }
}

function global:Stop-ScreenRecording {
    param(
        [string]$HostName = "LOCAL"
    )

    try {
        if ($global:RecordingSessions.ContainsKey($HostName)) {

            $rec = $global:RecordingSessions[$HostName]

            if ($rec.Process -and -not $rec.Process.HasExited) {
                Stop-Process -Id $rec.Process.Id -Force
            }

            global:Write-Terminal "Recording saved: $($rec.FilePath)" "Lime" $HostName

            $global:RecordingSessions.Remove($HostName)
        }
    }
    catch {
        global:Write-Terminal "Recording stop failed: $_" "Red" $HostName
    }
}
...
function global:Launch-RemoteBat($HostName, $Cred, $BatPath, $UserInput) {
    ...
    $sb = {
        ...
        $proc.Start() | Out-Null
        $null = global:Start-ScreenRecording -HostName $HostName -ScriptName "BatchExecution"
        ...
        Write-Output ">> [SYS] Output sync complete: global:Stop-ScreenRecording -HostName $HostName Screenshots and logs copied to RemotePayloads."
    }
