<#
.SYNOPSIS
    Installation Master Tool v7.1
    - SECURITY: Reverted ACLs back to Full Control (F) and removed Read-Only locks so scripts can be edited.
    - UI FIX: Dynamically anchored and layered the 'i' Info button to fix the disappearing issue caused by DPI scaling.
    - CORE: Auto-Download/Cleanup, O365 Deep Link, Timezone Clocks, and ADSI Telemetry retained.
#>
...
Write-Host "[INIT] Booting Installation Master Tool v7.1..." -ForegroundColor Cyan
...
# --- SECURITY LAYER REVERT: Restored to F (Full Control) so scripts remain editable ---
...
# --- UNBLOCK FILES (Read-Only Lock Removed) ---
...
$script:form.Add_Shown({ 
    ...
    foreach ($url in $DownloadUrls) {
        ...
        try {
            Write-Terminal "Attempting download from secure source..." "LightGray"
            ...
            Write-Terminal "Source failed, trying next availability zone..." "Yellow"
        }
    }
    ...
    # Path Hidden Here: Removed ($TempZip) from the logging message
    ...
    # APPLY FULL CONTROL ACLs (Fully Editable)
    Set-FolderPermissions $script:CurrentToolPath $false
    Unblock-ExtractedFiles $script:CurrentToolPath $false
