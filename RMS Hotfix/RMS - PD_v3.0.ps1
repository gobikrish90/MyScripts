<#
.SYNOPSIS
    MASTER ORCHESTRATOR: FINAL FIX (FORCED ID MAPPING)
    
.DESCRIPTION
    1. Auto-detects ProPhoenix Drive.
    2. Writes helper scripts (LogClear, Verification).
    3. Scans AppReg with FORCED MAPPING:
       - Fixes Fire/IA HelpDocs by forcing their IDs based on Name.
       - Deduplicates list to prevent multiple entries.
    4. Generates 'Production_Update.bat'.
    5. Launches the batch file.
#>

$ErrorActionPreference = "Continue"
Clear-Host
Write-Host "==========================================================" -ForegroundColor Cyan
Write-Host "   MASTER ORCHESTRATOR: FINAL (FORCED MAPPING)" -ForegroundColor Cyan
Write-Host "==========================================================" -ForegroundColor Cyan
...
# ==============================================================================
# 3. MAPPING DATA & SERVICE LIST
# ==============================================================================
Write-Host "`n[STEP 3] Loading Definitions..." -ForegroundColor Yellow
...
if ($xmlData.PhoenixApplications.AppReg) {
    foreach ($app in $xmlData.PhoenixApplications.AppReg) {
        ...
        # --- FORCE OVERRIDE FOR DOCS (FIXES MISSING FIRE/IA DOCS) ---
