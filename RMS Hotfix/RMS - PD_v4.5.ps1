<#
.SYNOPSIS
    MASTER ORCHESTRATOR: FINAL VISUAL
    
.DESCRIPTION
    1. APP MANAGER: Updates First (Step 0).
    2. EXCLUDED: RMS, Fire, IA, Hub, Gateway (Always Skipped).
    3. LOGIC 1: General Apps (Listed visually).
    4. LOGIC 2: WebServices, PDF, ReportWriter, Stage (Listed visually, stops services).
    5. LOGIC 3: HelpDocs (Listed visually).
    6. CLEANUP: Session Data Only.
    7. VERIFY: Full DLL Check.
#>
...
# B. VERIFICATION SCRIPT (ROBUST LOGIC)
$VerifyContent = @'
# -----------------------------------------------
...
Write-Host "   [CONFIG] EXCLUDED: RMS, Fire, IA, Hub, Gateway" -ForegroundColor DarkGray
...
# Lists for Display (Visual output)
$DisplayL1 = @()
$DisplayL2 = @()
$DisplayL3 = @()
...
if ($xmlData.PhoenixApplications.AppReg) {
    foreach ($app in $xmlData.PhoenixApplications.AppReg) {
        ...
        # --- EXCLUSION CHECK (RMS / Hub / Gateway) ---
        if ($ExcludedIDs -contains $UseID -or $Name -match "Hub" -or $Name -match "Gateway") { 
        ...
        # LOGIC 3 (FINAL STEP: HELP DOCS)
        if ($UseID -like "*Help*Doc*" -or $UseID -like "*Hazmat*" -or $Name -like "*Help*Doc*" -or $UseID -match "F1HelpDocs") {
            $ListL3.Add("`"$UseID`"")
            $DisplayL3 += $UseID
            Write-Host "$LogMsg -> LOGIC 3 (Docs)" -ForegroundColor Cyan
        }
        # LOGIC 2 (MAINTENANCE: STAGE, WEBSERVICES, PDF, REPORT WRITER)
        elseif ($Logic2IDs -contains $UseID -or $UseID -match "Stage" -or $UseID -like "*Web*Service*" -or $UseID -like "*PDF*") {
            $ListL2.Add("`"$UseID`"")
            $DisplayL2 += $UseID
            Write-Host "$LogMsg -> LOGIC 2 (Web/Stage/Special)" -ForegroundColor Magenta
        } 
        # LOGIC 1 (EVERYTHING ELSE)
        else {
            $ListL1.Add("`"$UseID`"")
            $DisplayL1 += $UseID
...
if (-not $StrL1Block) { $StrL1Block = "echo    (None)" }
if (-not $StrL2Block) { $StrL2Block = "echo    (None)" }
if (-not $StrL3Block) { $StrL3Block = "echo    (None)" }
...
:: ===================================================
:: LOGIC 2: SPECIAL APPS (WEB SERVICES / PDF / STAGE)
:: ===================================================
echo.
echo ---------------------------------------------------
echo  READY: LOGIC 2 (WEB SERVICES / PDF / STAGE)
echo ---------------------------------------------------
echo  The following products will be installed:
$StrL2Block
echo.
