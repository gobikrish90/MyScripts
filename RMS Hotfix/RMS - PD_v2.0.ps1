<#
.SYNOPSIS
    MASTER ORCHESTRATOR: DYNAMIC PHASE 2 WITH PERMISSIONS
    
.DESCRIPTION
    1. Prepares Workspace.
    2. Writes 'Hotfix Script.bat' (For Phase 1).
    3. Writes 'Minimal Downtime Script' (Phase 1).
    4. Launches Phase 1 -> Waits for Completion.
    5. SEARCHES Products -> Generates 'Phase 2 Batch' with strict prompts:
       - Prompt 1: "Start Logic 1?" (Services UP)
       - Prompt 2: "Start Logic 2?" (Services UP)
       - Prompt 3: "Start Maintenance?" (Stop Services)
    6. Launches Phase 2.
#>
...
Write-Host "   MASTER ORCHESTRATOR: PERMISSION-BASED UPDATE" -ForegroundColor Cyan
Write-Host "==========================================================" -ForegroundColor Cyan
...
# ==============================================================================
# A. BATCH: Hotfix Script.bat (Used by Phase 1)
$RMSBatContent = @'
@echo off
echo =================================================
...
# ==============================================================================
# 3. WRITE MINIMAL DOWNTIME SCRIPT (PHASE 1)
# ==============================================================================
...
foreach ($app in $appFolders) {
    ...
    if ($found) { 
        try {
            ...
            } else { Write-Host "   Found $app at $found (Access/Empty)" -ForegroundColor Gray }
        } catch { Write-Host "   Found $app at $found (Size Unknown)" -ForegroundColor Gray }
    }
}
...
# ==============================================================================
# 5. SEARCH & GENERATE PHASE 2 BATCH
# ==============================================================================
Write-Host "`n[STEP 5] Searching Products & Generating Batch..." -ForegroundColor Yellow
...
# BATCH CONTENT WITH PERMISSION PROMPTS
$BatchContent = @"
$InstallDrive
cd "$AppMgrFolder"
@echo off
...
echo ---------------------------------------------------
:AskL2
SET /P "StartL2=>> Start Logic 2 Install now? (Y/N): "
...
echo.
echo ---------------------------------------------------
echo  READY: MAINTENANCE MODE
echo  WARNING: This will STOP SERVICES!
echo ---------------------------------------------------
:AskMaint
SET /P "StartMaint=>> Stop Services and Update Instances? (Y/N): "
IF /I "%StartMaint%" NEQ "Y" GOTO AskMaint
