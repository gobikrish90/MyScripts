# ======================================================================
#  ProPhoenix DB Utility Dashboard - v6.7
# ======================================================================
...
# --- HEADER (TIGHT LEFT-ALIGNED LOGO & TEXT) ---
...
# --- REUSABLE SYNC FUNCTION (RAW STRING XML GENERATOR FIX) ---
function Execute-DBSync {
    ...
    foreach ($i in $Items) {
        ...
        try {
            ...
            # --- DYNAMIC RAW XML GENERATION (Fixes "Auto Remove" execution failure) ---
            $XmlPath = Join-Path $WD "PnxAutoNewDBSyn.xml"
            if (Test-Path $XmlPath) { Remove-Item $XmlPath -Force }

            $XmlString = "<?xml version=`"1.0`" encoding=`"utf-8`" ?>`r`n"
            $XmlString += "<PnxPakager>`r`n"
            $XmlString += "`t<SourceServer>`r`n"
            $XmlString += "`t`t<IPAddress>$($txtS.Text)</IPAddress>`r`n"
            $XmlString += "`t`t<DBName>$D</DBName>`r`n"
            $XmlString += "`t`t<UserName>$($txtU.Text)</UserName>`r`n"
            $XmlString += "`t`t<Password>$($txtP.Text)</Password>`r`n"
            $XmlString += "`t`t<JurisID>1000</JurisID>`r`n"
            $XmlString += "`t`t<State>MA</State>`r`n"
            $XmlString += "`t`t<JurisName>ProPhoenix</JurisName>`r`n"
            $XmlString += "`t`t<JurisAlias>PNX</JurisAlias>`r`n"
            $XmlString += "`t`t<SyncType>2</SyncType>`r`n"
            $XmlString += "`t</SourceServer>`r`n"
            $XmlString += "</PnxPakager>"

            # Write without UTF-8 BOM to ensure compatibility with C# tool
...
"@
        ...
        foreach ($job in $restoreJobs) {
            ...
            # STRICT JOB REPLACEMENT (Deletes old params/notifies first)
            $jobQuery = @"
            DECLARE @jID BIGINT;
...
# --- COPY DB (WITH ENTERPRISE CONFIG PROMPT) ---
...
$btnCopyDB.Add_Click({
    ...
    try {
        ...
        # WITH REPLACE inherently bypasses tail-log backups
...
# --- ENTERPRISE BACKUP PARAMS (NOTEPAD .TXT FORMAT) ---
$btnBackup.Add_Click({
    if($listDBs.CheckedItems.Count -eq 0){ [System.Windows.Forms.MessageBox]::Show("Select DBs to Backup!"); return }
    ...
    foreach ($item in $listDBs.CheckedItems) {
        ...
        try {
            ...
            foreach ($job in $tempJobs) {
                ...
                while ($pReader.Read()) {
                    # STRICT NULL CHECKS TO PREVENT "DATA IS NULL" EXCEPTION
                ...
                # Robust Ordinal Notify Extraction
...
# --- ENTERPRISE RESTORE PARAMS (NOTEPAD .TXT FORMAT) ---
$btnRestore.Add_Click({
    if($listDBs.CheckedItems.Count -eq 0){ [System.Windows.Forms.MessageBox]::Show("Select DBs to Restore!"); return }
