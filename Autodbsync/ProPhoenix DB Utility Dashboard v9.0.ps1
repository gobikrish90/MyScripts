...
# ======================================================================
#  ProPhoenix DB Utility Dashboard v8.8
# ======================================================================
...
$Script:form.Text = "ProPhoenix DB Utility Dashboard v8.8"
...
# ======================================================================
# --- COPY DB (LIVE TO TRAIN/TEST) - MISSING FUNCTION RESOLVED ---
# ======================================================================

function Show-CopyDialog {
    $d = New-Object System.Windows.Forms.Form
    $d.Text = "Select Copy Type"
    $d.Size = New-Object System.Drawing.Size(300, 150)
    $d.StartPosition = "CenterParent"
    
    $l = New-Object System.Windows.Forms.Label
    $l.Text = "Select Target Type:"
    $l.Location = "20,20"
    $l.AutoSize = $true
    $d.Controls.Add($l)
    
    $cb = New-Object System.Windows.Forms.ComboBox
    $cb.Items.AddRange(@("Train (Tr)", "Test (Test)"))
    $cb.SelectedIndex = 0
    $cb.Location = "20,50"
    $cb.Width = 240
    $d.Controls.Add($cb)
    ...
    if ($d.ShowDialog($Script:form) -eq [System.Windows.Forms.DialogResult]::OK) { 
...
$Script:btnCopyDB.Add_Click({
    try {
        ...
        # --- 1. SQL-SIDE DATABASE EXISTENCE CHECK ---
        $CS="Server=$($Script:txtS.Text);Database=master;User Id=$($Script:txtU.Text);Password=$($Script:txtP.Text);Connection Timeout=30"
        $CN=New-Object System.Data.SqlClient.SqlConnection($CS); $CN.Open()
        $Cmd=$CN.CreateCommand(); $Cmd.CommandTimeout = 0
        
        $Cmd.CommandText = "SELECT ISNULL(DB_ID('$TargetDB'), 0)"
        $targetId = $Cmd.ExecuteScalar()
            ...
            if ($ansBkp.Equals([System.Windows.Forms.DialogResult]::Yes)) {
                ...
                if (![string]::IsNullOrWhiteSpace($selJ)) {
                    ...
                    if($fbdBkp.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK){
                        ...
                        $cmdTarget.CommandText = "SET NOCOUNT ON; SELECT ParamID, CAST(ParamValue AS NVARCHAR(MAX)) FROM Parameter WHERE ParamID IN ($($Script:TargetParams -join ',')) AND JurisID=$selJ"
