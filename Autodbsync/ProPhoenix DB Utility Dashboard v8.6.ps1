...
# ======================================================================
#  ProPhoenix DB Utility Dashboard v8.6
# ======================================================================
...
$Script:form.Text = "ProPhoenix DB Utility Dashboard v8.6"
...
# --- EXTRACTED INPUT BOX FUNCTION FOR PARAMETERS JURIS ID ---
function Show-InputBox { 
    param($T, $P, $D) 
    $f = New-Object System.Windows.Forms.Form
    $f.Text = $T
    $f.Size = New-Object System.Drawing.Size(350, 150)
    ...
    $t.Text = $D
    $t.Location = New-Object System.Drawing.Point(10, 35)
    $t.Width = 310
    $f.Controls.Add($t)
    $b = New-Object System.Windows.Forms.Button
    $b.Text = "OK"
    $b.DialogResult = "OK"
    $b.Location = New-Object System.Drawing.Point(120, 70)
    $f.Controls.Add($b)
    $f.AcceptButton = $b
...
# --- NEW: FETCH JURIS IDs & DROPDOWN PROMPT ---
function Get-DBJurisIDs {
    param($DBName)
    $jList = @()
    try {
        $CS="Server=$($Script:txtS.Text);Database=$DBName;User Id=$($Script:txtU.Text);Password=$($Script:txtP.Text);Connection Timeout=5"
    ...
    if ($jList.Count -eq 0) { $jList += "1000" } # Fallback if empty or query fails
    return $jList
}
...
function Show-JurisSelector { 
    ...
    $f = New-Object System.Windows.Forms.Form; $f.Text = $T; $f.Size = New-Object System.Drawing.Size(350, 150); $f.StartPosition = "CenterParent"
    $l = New-Object System.Windows.Forms.Label; $l.Text = $P; $l.Location = "10,10"; $l.AutoSize = $true; $f.Controls.Add($l)
    $cb = New-Object System.Windows.Forms.ComboBox; $cb.Location = "10,35"; $cb.Width = 310
    foreach($o in $Options){ [void]$cb.Items.Add($o) }
    if($cb.Items.Count -gt 0){ $cb.SelectedIndex = 0 }
    $f.Controls.Add($cb)
    $b = New-Object System.Windows.Forms.Button; $b.Text = "OK"
    $b.DialogResult = [System.Windows.Forms.DialogResult]::OK 
    $b.Location = "120,70"; $f.Controls.Add($b); $f.AcceptButton = $b
...
# --- DELETE DATABASE LOGIC (FAIL-SAFE) ---
$Script:btnDeleteDB.Add_Click({
    try {
        if($Script:listDBs.CheckedItems.Count -eq 0){ 
            [System.Windows.Forms.MessageBox]::Show($Script:form, "Select at least one database to delete from the list!", "No Selection", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning) | Out-Null
            return 
        }

        # --- 1. CRITICAL CONFIRMATION ---
        $count = $Script:listDBs.CheckedItems.Count
        $msg = "You are about to PERMANENTLY DELETE ($count) database(s).`n`nThis action is irreversible and will erase all data.`n`nDo you want to proceed?"
        $ans = [System.Windows.Forms.MessageBox]::Show($Script:form, $msg, "CRITICAL WARNING", [System.Windows.Forms.MessageBoxButtons]::YesNo, [System.Windows.Forms.MessageBoxIcon]::Stop)
        
        if($ans -ne [System.Windows.Forms.DialogResult]::Yes){ return }

        # Final safety check if multiple DBs are selected
        if($count -gt 1) {
             if([System.Windows.Forms.MessageBox]::Show($Script:form, "Confirm again: Delete ALL $count selected databases?", "Bulk Delete Confirmation", [System.Windows.Forms.MessageBoxButtons]::OKCancel, [System.Windows.Forms.MessageBoxIcon]::Warning) -ne [System.Windows.Forms.DialogResult]::OK) { return }
        }

        Toggle $false
        Log "▶ INITIATING DATABASE DELETION..." "Red"

        # --- 2. EXECUTE DROP COMMANDS ---
        $CS = "Server=$($Script:txtS.Text);Database=master;User Id=$($Script:txtU.Text);Password=$($Script:txtP.Text);Connection Timeout=30"
        $CN = New-Object System.Data.SqlClient.SqlConnection($CS); $CN.Open()

        foreach ($item in $Script:listDBs.CheckedItems) {
            $DB = $Script:TargetMap[$item].DB
            Log "  > Dropping Database: [$DB]..." "Orange"; [System.Windows.Forms.Application]::DoEvents()

            try {
                $Cmd = $CN.CreateCommand()
                $Cmd.CommandTimeout = 0
                # Force kick users and Drop
                $Cmd.CommandText = @"
...
"@
                ...
                Log "  ❌ Failed to delete $DB : $($_.Exception.Message)" "Red"
            }
        }
        $CN.Close()
        
        Log "✔ Deletion process finished." "Cyan"
        
        # --- 3. REFRESH UI ---
        $Script:btnCon.PerformClick() 
    } 
    catch { 
        Log "❌ CRITICAL ERROR during deletion: $($_.Exception.Message)" "Red" 
    } 
    finally { 
        Toggle $true 
    }
...
function Apply-EnterpriseConfig {
    ...
    try {
        $CS="Server=$($Script:txtS.Text);Database=$TargetDB;User Id=$($Script:txtU.Text);Password=$($Script:txtP.Text);Connection Timeout=15"
        $CN=New-Object System.Data.SqlClient.SqlConnection($CS)
        $CN.Open()
        ...
        foreach ($paramID in $restoreParams.Keys) {
            ...
            # --- HIGH-PRECISION JURIS TARGETING & TYPE CASTING ---
            $upsertQuery = @"
            SET NOCOUNT ON;
...
"@
        ...
        foreach ($job in $restoreJobs) {
            ...
            $cmd = $CN.CreateCommand(); $cmd.CommandText = $jobQuery
...
# --- COPY DB (LIVE TO TRAIN/TEST) - NULL-SAFE WORKFLOW ---
$Script:btnCopyDB.Add_Click({
    try {
        ...
        # --- 1. PRE-OVERWRITE LOGIC ---
        $CS="Server=$($Script:txtS.Text);Database=master;User Id=$($Script:txtU.Text);Password=$($Script:txtP.Text);Connection Timeout=30"
        $CN=New-Object System.Data.SqlClient.SqlConnection($CS); $CN.Open()
        $Cmd=$CN.CreateCommand(); $Cmd.CommandTimeout = 0
        $Cmd.CommandText = "SELECT DB_ID('$TargetDB')"; $targetExists = $Cmd.ExecuteScalar()

        if ($null -ne $targetExists -and $targetExists -gt 0) {
            $ansBkp = [System.Windows.Forms.MessageBox]::Show("Target database [$TargetDB] already exists.`n`nDo you want to take a backup of its existing Parameters and Jobs before overwriting?", "Backup Existing Target?", [System.Windows.Forms.MessageBoxButtons]::YesNo, [System.Windows.Forms.MessageBoxIcon]::Question)
            if ($ansBkp -eq [System.Windows.Forms.DialogResult]::Yes) {
                
                $availableJuris = Get-DBJurisIDs -DBName $TargetDB
                $inputJuris = Show-JurisSelector "Source JurisID" "Select or Enter JurisID to Backup from [$TargetDB]:" $availableJuris
                
                if ([string]::IsNullOrWhiteSpace($inputJuris) -or -not ($inputJuris -match '^\d+$')) { $CN.Close(); return }
                
                $fbdBkp = New-Object System.Windows.Forms.FolderBrowserDialog; $fbdBkp.Description = "Select Folder to Save Pre-Overwrite Backup"; $fbdBkp.SelectedPath = $Script:DefaultBackup
                if($fbdBkp.ShowDialog() -ne [System.Windows.Forms.DialogResult]::OK){ $CN.Close(); return }
                
                Log "▶ BACKING UP EXISTING TARGET JURIS $inputJuris FROM $TargetDB..." "Cyan"
                ...
                $CSTarget="Server=$($Script:txtS.Text);Database=$TargetDB;User Id=$($Script:txtU.Text);Password=$($Script:txtP.Text);Connection Timeout=15"
                $CNTarget=New-Object System.Data.SqlClient.SqlConnection($CSTarget); $CNTarget.Open()
                
                $cmdTarget = $CNTarget.CreateCommand()
                $cmdTarget.CommandText = "SET NOCOUNT ON; SELECT ParamID, CAST(ParamValue AS NVARCHAR(MAX)) AS ParamValue FROM Parameter WHERE ParamID IN ($($Script:TargetParams -join ',')) AND JurisID=$inputJuris"
                $reader = $cmdTarget.ExecuteReader(); $BackupLines += "[PARAMETERS]"
                # FIX: Added IsDBNull checks to prevent GetString crashes on empty parameters
                while ($reader.Read()) { 
                    $pVal = if ($reader.IsDBNull(1)) { "" } else { $reader.GetString(1) }
                    $BackupLines += "$($reader.GetValue(0))=$pVal" 
                }
                ...
                $jobReader = $cmdTarget.ExecuteReader(); $tempJobs = @(); 
                while ($jobReader.Read()) { 
                    $tempJobs += @{ ID = $jobReader.GetValue(0); Name = $jobReader.GetString(1); Type = if ($jobReader.IsDBNull(2)) { "" } else { $jobReader.GetString(2) } } 
                }
                $jobReader.Close()
                
                foreach ($job in $tempJobs) { 
                    $BackupLines += "[JOB:$($job.Name)]"; if ($job.Type -ne "") { $BackupLines += "JobType=$($job.Type)" }
                    $cmdTarget.CommandText = "SELECT ParamName, ParamValue FROM KPIjobsparam WHERE JobID = $($job.ID)"
                    $pReader = $cmdTarget.ExecuteReader() 
                    # FIX: Added IsDBNull checks to prevent GetString crashes on empty job parameters
                    while ($pReader.Read()) { 
                        $jpName = if ($pReader.IsDBNull(0)) { "" } else { $pReader.GetString(0) }
                        $jpVal = if ($pReader.IsDBNull(1)) { "" } else { $pReader.GetString(1) }
                        $BackupLines += "Param:$jpName=$jpVal" 
                    }
                    $pReader.Close()
                    
                    $cmdTarget.CommandText = "SELECT * FROM KPIJobsNotify WHERE JobID = $($job.ID)"
                    $nReader = $cmdTarget.ExecuteReader()
                    while ($nReader.Read()) { if (!($nReader.IsDBNull(4))) { $BackupLines += "NotifyPath=$($nReader.GetString(4))" } }
                    $nReader.Close(); $BackupLines += "" 
                }
                [IO.File]::WriteAllLines((Join-Path $fbdBkp.SelectedPath "$($TargetDB)_PreCopyBackup_$TS.txt"), $BackupLines, [System.Text.Encoding]::UTF8)
                $CNTarget.Close()
                Log "  ✔ Pre-Overwrite Backup Saved." "Lime"
            }

            $ansDrop = [System.Windows.Forms.MessageBox]::Show("WARNING: You are about to OVERWRITE and completely replace the existing database: $TargetDB.`n`nProceed?", "Confirm Overwrite", [System.Windows.Forms.MessageBoxButtons]::YesNo, [System.Windows.Forms.MessageBoxIcon]::Error)
            if ($ansDrop -ne [System.Windows.Forms.DialogResult]::Yes) { $CN.Close(); return }
        }

        # --- 2. POST-RESTORE CONFIGURATION PROMPT ---
        $ansRest = [System.Windows.Forms.MessageBox]::Show("Do you want to apply an Enterprise Configuration Backup (.txt) to the newly copied database ($TargetDB) after the copy completes?", "Apply Configuration?", [System.Windows.Forms.MessageBoxButtons]::YesNo, [System.Windows.Forms.MessageBoxIcon]::Question)
        $ConfigFilePath = $null
        $TargetJuris = "1000"
        
        if ($ansRest -eq [System.Windows.Forms.DialogResult]::Yes) {
            $f = New-Object System.Windows.Forms.OpenFileDialog
            ...
            if ($f.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) { 
                ...
                $TargetJuris = Show-InputBox "Target JurisID" "Enter the Target JurisID for restoring params into [$TargetDB]:" "1000"
                if ([string]::IsNullOrWhiteSpace($TargetJuris) -or -not ($TargetJuris -match '^\d+$')) { $CN.Close(); return }
            }
        }

        # --- 3. COPY DB EXECUTION ---
        Toggle $false; Log "▶ COPYING DATABASE: $SourceDB to $TargetDB..." "Cyan"
        $CN.FireInfoMessageEventOnUserErrors = $true
        $CN.add_InfoMessage({ param($sender, $e) if ($e.Message -match "percent processed" -or $e.Message -match "processed.") { Log "   > $($e.Message)" "DarkGray" } })
        ...
        $BakFile = Join-Path $fbd.SelectedPath "$SourceDB`_Copy.bak"

        Log "  > Backing up to $BakFile..."; [System.Windows.Forms.Application]::DoEvents()
        ...
        try { 
            $CmdDrop = New-Object System.Data.SqlClient.SqlCommand("IF EXISTS(SELECT 1 FROM sys.databases WHERE name='$TargetDB') BEGIN DECLARE @kill varchar(8000) = ''; SELECT @kill = @kill + 'kill ' + CONVERT(varchar(5), session_id) + ';' FROM sys.dm_exec_sessions WHERE database_id = db_id('$TargetDB'); EXEC(@kill); ALTER DATABASE [$TargetDB] SET SINGLE_USER WITH ROLLBACK IMMEDIATE; END", $CN)
            $CmdDrop.CommandTimeout = 0; $CmdDrop.ExecuteNonQuery()|Out-Null 
        } catch { }

        Log "  > Restoring..."; [System.Windows.Forms.Application]::DoEvents()
        $Cmd.CommandText="RESTORE FILELISTONLY FROM DISK='$BakFile'"; $Rdr=$Cmd.ExecuteReader(); $Files=@(); while($Rdr.Read()){$Files+=@{L=$Rdr["LogicalName"];P=$Rdr["PhysicalName"];T=$Rdr["Type"]}}; $Rdr.Close()
        ...
        $Cmd.CommandText="RESTORE DATABASE [$TargetDB] FROM DISK='$BakFile' WITH RECOVERY, REPLACE, STATS=10, $($MoveStr.TrimEnd(', '))"
        $Cmd.ExecuteNonQuery()|Out-Null; $Cmd.CommandText = "ALTER DATABASE [$TargetDB] SET MULTI_USER"; $Cmd.ExecuteNonQuery()|Out-Null
        Log "  ✔ Restore Complete" "Lime"

        # --- 4. APPLY TARGETED CONFIGURATION ---
        if ($ConfigFilePath -and (Test-Path $ConfigFilePath)) {
            Log "▶ Applying Config to $TargetDB (Juris: $TargetJuris)..." "Yellow"
            Apply-EnterpriseConfig -TargetDB $TargetDB -SourceFile $ConfigFilePath -TargetJuris $TargetJuris
        } else {
            Log "▶ Running Default Hardcoded Cleanup..." "Yellow"
            ...
            $DelCmd = $CleanCN.CreateCommand(); $DelCmd.CommandTimeout = 0
            ...
            if ($SourceDB -match "Police") { $DelCmd.CommandText += " DELETE FROM Parameter WHERE ParamID IN (203, 204, 205, 206, 207, 222); " } 
            elseif ($SourceDB -match "Fire") { $DelCmd.CommandText += " DELETE FROM Parameter WHERE ParamID IN (203, 204, 205, 206, 207, 221); " }
            $DelCmd.ExecuteNonQuery()|Out-Null; $CleanCN.Close()
            Log "  ✔ Final Default Cleanup Done" "Lime"
        }

        $CN.Close(); Log "✔ All steps for [$TargetDB] finished." "Lime"; $Script:btnCon.PerformClick()
    } 
...
$Script:btnBackup.Add_Click({
    try {
        if($Script:listDBs.CheckedItems.Count -eq 0){ return }
        
        $FirstDB = $Script:TargetMap[$Script:listDBs.CheckedItems[0]].DB
        ...
        foreach ($item in $Script:listDBs.CheckedItems) {
            $DB = $Script:TargetMap[$item].DB; Log "  > Backing up Config for $DB..."; $BackupLines = @("# ProPhoenix Database Configuration Backup", "# Database: $DB", "# Timestamp: $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')", "")
            $CS="Server=$($Script:txtS.Text);Database=$DB;User Id=$($Script:txtU.Text);Password=$($Script:txtP.Text);Connection Timeout=15"; $CN=New-Object System.Data.SqlClient.SqlConnection($CS); $CN.Open()
            $cmd = $CN.CreateCommand(); $cmd.CommandText = "SET NOCOUNT ON; SELECT ParamID, CAST(ParamValue AS NVARCHAR(MAX)) AS ParamValue FROM Parameter WHERE ParamID IN ($($Script:TargetParams -join ',')) AND JurisID=$inputJuris"
            $reader = $cmd.ExecuteReader(); $BackupLines += "[PARAMETERS]"; $paramCount = 0; while ($reader.Read()) { $BackupLines += "$($reader.GetValue(0))=$($reader.GetString(1))"; $paramCount++ }; $reader.Close(); Log "    + Exported $paramCount Parameters"; $BackupLines += ""
            $cmd.CommandText = "SELECT JobID, JobName, JobType FROM KPIjobs WHERE JobName IN ('$($Script:TargetJobs -join "','")')"; $jobReader = $cmd.ExecuteReader(); $tempJobs = @(); while ($jobReader.Read()) { $tempJobs += @{ ID = $jobReader.GetValue(0); Name = $jobReader.GetString(1); Type = if ($jobReader.IsDBNull(2)) { "" } else { $jobReader.GetString(2) } } }; $jobReader.Close()
            foreach ($job in $tempJobs) { $BackupLines += "[JOB:$($job.Name)]"; if ($job.Type -ne "") { $BackupLines += "JobType=$($job.Type)" }; $cmd.CommandText = "SELECT ParamName, ParamValue FROM KPIjobsparam WHERE JobID = $($job.ID)"; $pReader = $cmd.ExecuteReader(); while ($pReader.Read()) { $BackupLines += "Param:$($pReader.GetString(0))=$($pReader.GetString(1))" }; $pReader.Close(); 
            $cmd.CommandText = "SELECT * FROM KPIJobsNotify WHERE JobID = $($job.ID)"; $nReader = $cmd.ExecuteReader(); while ($nReader.Read()) { if (!($nReader.IsDBNull(4))) { $BackupLines += "NotifyPath=$($nReader.GetString(4))" } }; $nReader.Close(); $BackupLines += "" }
            Log "    + Exported $($tempJobs.Count) KPI Jobs"; [IO.File]::WriteAllLines((Join-Path $fbd.SelectedPath "$($DB)_ConfigBackup_$TS.txt"), $BackupLines, [System.Text.Encoding]::UTF8); $CN.Close()
        }
...
$Script:btnRestore.Add_Click({
    try {
        ...
        foreach ($item in $Script:listDBs.CheckedItems) { 
            $DB=$Script:TargetMap[$item].DB; Log "  > Target: $DB"; 
