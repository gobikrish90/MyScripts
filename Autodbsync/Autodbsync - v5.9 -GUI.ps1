# ======================================================================
#  ProPhoenix DB Utility Dashboard - v145.0 (Real-Time Progress & Live Tr)
# ======================================================================
...
# Known Jobs to Scan
$Script:KnownJobs = @(
    "WDAAppDataExporter", "ReportWriterStaticDataExporter", "FireLiveDataExporter", 
    "PhoenixBOTQAUploader", "KPICleaner", "CADStaticDataExtractor", "Hot Sheet", 
    "FireRMSDataExporter"
)
...
$form.Text = "ProPhoenix DB Utility Dashboard v145"
...
function Execute-DBSync {
    ...
    foreach ($i in $Items) {
        ...
        try {
            if ($F -eq "None") { throw "No Utility Folder Mapped" }
            $WD = "$($Script:DBSyncRoot)\$F"
            if (!(Test-Path "$WD\PnxDBSync.exe")) { throw "Missing EXE in $WD" }
            ...
            Log "   ✔ Done" "Lime"
        } catch { Log "   ! Skipped: $($_.Exception.Message)" "Orange" }
    }
}

# --- CONNECT (AUTOMATION & LIVE/TEST FIX) ---
$btnCon.Add_Click({
    ...
    try {
        ...
        if ($chkAutoSync.Checked -and $Script:AppMgrPath -and (Test-Path $Script:AppMgrPath)) {
            ...
            Log "✔ AppMgr Refreshed." "Lime"
        }

        foreach ($row in $ds.Tables[0].Rows) {
            $db = $row.Name; $Folder = $null; $Tag = ""; $Type = ""
            
            # --- FIX: Training DBs are LIVE ---
            $IsTest = ($db -match "Test" -or $db -match "Automation" -or $db -match "Arch" -or $db -match "Archive")
            $Cat = if ($IsTest) { "TEST" } else { "LIVE" } # "Tr" and "Train" fall into LIVE
...
# --- AUTO-DISCOVERY FUNCTION (Schema Agnostic) ---
function Export-JobConfig {
    param($DB, $Folder)
    $ConfigLines = @()
    try {
        ...
        $Check=$CN.CreateCommand(); $Check.CommandText="SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME='KPIJobs'"; if($Check.ExecuteScalar()-eq 0){ $CN.Close(); return }

        foreach ($Job in $Script:KnownJobs) {
            $Cmd=$CN.CreateCommand(); $Cmd.CommandText="SELECT JobID FROM KPIJobs WHERE JobName='$Job'"; $JobID=$Cmd.ExecuteScalar()
            if ($JobID) {
                $ConfigLines += "[$Job]"
                $Cmd.CommandText="SELECT ParamValue FROM KPIJobsParam WHERE JobID=$JobID AND ParamName='Instance'"; $Inst=$Cmd.ExecuteScalar()
                if ($Inst) { $ConfigLines += "Instance=$Inst" }
                
                $Cmd.CommandText="SELECT * FROM KPIJobsNotify WHERE JobID=$JobID"; $DA=New-Object System.Data.SqlClient.SqlDataAdapter($Cmd); $DT=New-Object System.Data.DataTable; $DA.Fill($DT)|Out-Null
                $fc=1
                foreach ($Row in $DT.Rows) {
                    foreach ($Col in $DT.Columns) {
                        $Val = $Row[$Col].ToString()
                        if ($Val -match "^\\\\|^[a-zA-Z]:\\") {
                            $ConfigLines += "Folder$fc=$Val"; $fc++; break 
                        }
                    }
                }
                $ConfigLines += ""
        ...
        if ($ConfigLines.Count -gt 0) {
            $Path = Join-Path $Folder "$($DB)_JobConfig.txt"; $ConfigLines | Out-File $Path -Encoding UTF8
            Log "   + Config Exported: $(Split-Path $Path -Leaf)" "Gray"
        } else { Log "   - No Active Job Configs Found" "Yellow" }
    } catch { Log "   ! Config Export Failed: $($_.Exception.Message)" "Red" }
}

# --- COPY DB (With Real-Time Progress & Safe Restores) ---
...
$btnCopyDB.Add_Click({
    ...
    try {
        ...
        # --- NEW: REAL-TIME PROGRESS LISTENER ---
        $CN.FireInfoMessageEventOnUserErrors = $true
        $CN.add_InfoMessage({
            param($sender, $e)
            if ($e.Message -match "percent processed" -or $e.Message -match "processed.") {
                Log "   > $($e.Message)" "DarkGray"
            }
        })
        $CN.Open()
        ...
        Export-JobConfig $SourceDB $BackupPath
        ...
        $ConfigFile = Join-Path $BackupPath "$($SourceDB)_JobConfig.txt"
        if (Test-Path $ConfigFile) {
            ...
            $CurJob=""; Get-Content $ConfigFile | ForEach-Object {
                $Line=$_.Trim(); if($Line -match "^\[(.+)\]$"){$CurJob=$Matches[1]}
                elseif($Line -match "Instance=(.+)") {
                    $Inst=$Matches[1]; $Q="UPDATE KPIJobs SET IsInactive=1 WHERE JobName='$CurJob'; INSERT INTO KPIjobsparam(JobID,SeqNo,ParamName,ParamValue) SELECT JobID,(SELECT ISNULL(MAX(SeqNo),0)+1 FROM KPIjobsparam WHERE JobID=j.JobID),'Instance','$Inst' FROM KPIJobs j WHERE JobName='$CurJob'"; $C=$CleanCN.CreateCommand();$C.CommandText=$Q;$C.ExecuteNonQuery()|Out-Null
                } elseif($Line -match "Folder\d*=(.+)") {
                    $Fld=$Matches[1]; $Q="INSERT INTO KPIJobsNotify SELECT JobID,(SELECT ISNULL(MAX(SeqNo),0)+1 FROM KPIJobsNotify WHERE JobID=j.JobID),NULL,NULL,'$Fld',1,NULL,NULL,NULL,NULL FROM KPIJobs j WHERE JobName='$CurJob'"; $C=$CleanCN.CreateCommand();$C.CommandText=$Q;$C.ExecuteNonQuery()|Out-Null
                }
            }
            $CleanCN.Close(); Log "   + Config Applied" "Gray"
        }
...
$btnBackup.Add_Click({
    ...
    foreach ($item in $listDBs.CheckedItems) {
        ...
        try {
            ...
            $CmdParam=$CN.CreateCommand(); $CmdParam.CommandText="SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME IN ('Parameter','ParameterName')"; $Tbl=$CmdParam.ExecuteScalar()
            if($Tbl){
...
$btnRestore.Add_Click({
    ...
    $ConfigFile = $SourceFile.Replace("_GlobalParameters.txt", "_JobConfig.txt")
    
    $Msg = "RESTORE from:\n$SourceFile"
    if ($JobPath) { $Msg += "\n\nAND Job CSV Data:\n$($JobPath.Name)" }
    if (Test-Path $ConfigFile) { $Msg += "\n\nAND Config File:\n$(Split-Path $ConfigFile -Leaf)" }
        ...
        try {
            ...
            if (Test-Path $ConfigFile) {
                $CurJob=""; Get-Content $ConfigFile | ForEach-Object {
                    $Line=$_.Trim(); if($Line -match "^\[(.+)\]$"){$CurJob=$Matches[1]}
                    elseif($Line -match "Instance=(.+)") {
                        $Inst=$Matches[1]; $Q="UPDATE KPIJobs SET IsInactive=1 WHERE JobName='$CurJob'; INSERT INTO KPIjobsparam(JobID,SeqNo,ParamName,ParamValue) SELECT JobID,(SELECT ISNULL(MAX(SeqNo),0)+1 FROM KPIjobsparam WHERE JobID=j.JobID),'Instance','$Inst' FROM KPIJobs j WHERE JobName='$CurJob'"; $C=$CN.CreateCommand();$C.CommandText=$Q;$C.ExecuteNonQuery()|Out-Null
                    } elseif($Line -match "Folder\d*=(.+)") {
                        $Fld=$Matches[1]; $Q="INSERT INTO KPIJobsNotify SELECT JobID,(SELECT ISNULL(MAX(SeqNo),0)+1 FROM KPIJobsNotify WHERE JobID=j.JobID),NULL,NULL,'$Fld',1,NULL,NULL,NULL,NULL FROM KPIJobs j WHERE JobName='$CurJob'"; $C=$CN.CreateCommand();$C.CommandText=$Q;$C.ExecuteNonQuery()|Out-Null
                    }
                }
                Log "   ✔ Job Config Paths Restored" "Lime"
            }
