...
# ======================================================================
#  ProPhoenix DB Utility Dashboard - v8.0 (Ultimate Codebook Pipeline)
# ======================================================================
...
$colBg = [System.Drawing.Color]::FromArgb(15, 15, 15) # Deep fallback base
$colText = [System.Drawing.Color]::WhiteSmoke
$colInputBg = [System.Drawing.Color]::FromArgb(25, 25, 25) # Sleek solid dark for input fields
$colGlassBtn = [System.Drawing.Color]::FromArgb(160, 30, 30, 30) # Translucent button glass
$colBorder = [System.Drawing.Color]::FromArgb(80, 255, 255, 255) # Frosted white borders
...
$form.Text = "ProPhoenix DB Utility Dashboard v8.0"
...
$lblVerDisplay = New-Object System.Windows.Forms.Label; $lblVerDisplay.Text="DB Ver: --"; $lblVerDisplay.AutoSize=$false; $lblVerDisplay.TextAlign="MiddleRight"; $lblVerDisplay.Size="300,20"; $lblVerDisplay.Location="965,42"; $lblVerDisplay.ForeColor="LightGray"; $lblVerDisplay.BackColor=[System.Drawing.Color]::Transparent; $pnlHead.Controls.Add($lblVerDisplay)
$Script:lblCBVerDisplay = New-Object System.Windows.Forms.Label; $Script:lblCBVerDisplay.Text="CB Ver: --"; $Script:lblCBVerDisplay.AutoSize=$false; $Script:lblCBVerDisplay.TextAlign="MiddleRight"; $Script:lblCBVerDisplay.Size="300,20"; $Script:lblCBVerDisplay.Location="965,65"; $Script:lblCBVerDisplay.ForeColor="LightGray"; $Script:lblCBVerDisplay.BackColor=[System.Drawing.Color]::Transparent; $pnlHead.Controls.Add($Script:lblCBVerDisplay)
...
# NEW CODEBOOK & DB BACKUP/RESTORE ROW
$Script:btnCodebook = New-Object System.Windows.Forms.Button; $Script:btnCodebook.Text="CODEBOOK Sync"; $Script:btnCodebook.Dock="Fill"; Set-MirrorButton $Script:btnCodebook ([System.Drawing.Color]::Crimson); $Script:btnCodebook.Margin=$btnMargin; $pnlAct.Controls.Add($Script:btnCodebook, 0, 2)
$Script:btnBackupDB = New-Object System.Windows.Forms.Button; $Script:btnBackupDB.Text="BACKUP DB"; $Script:btnBackupDB.Dock="Fill"; Set-MirrorButton $Script:btnBackupDB ([System.Drawing.Color]::SeaGreen); $Script:btnBackupDB.Margin=$btnMargin; $pnlAct.Controls.Add($Script:btnBackupDB, 1, 2)
$Script:btnRestoreDB = New-Object System.Windows.Forms.Button; $Script:btnRestoreDB.Text="RESTORE DB"; $Script:btnRestoreDB.Dock="Fill"; Set-MirrorButton $Script:btnRestoreDB ([System.Drawing.Color]::IndianRed); $Script:btnRestoreDB.Margin=$btnMargin; $pnlAct.Controls.Add($Script:btnRestoreDB, 2, 2)

# --- STATUS ---
$stat = New-Object System.Windows.Forms.StatusStrip; $stat.BackColor=[System.Drawing.Color]::FromArgb(200, 10, 10, 10); $lblStat = New-Object System.Windows.Forms.ToolStripStatusLabel; $lblStat.Text="Ready."; $lblStat.ForeColor="White"; $stat.Items.Add($lblStat); [void]$masterGrid.Controls.Add($stat, 0, 5)
...
function Scan-Server {
    ...
    if ($Result.Valid) {
        ...
        $lblVerDisplay.Text = "DB Ver: $($Result.Version)"; $lblVerDisplay.ForeColor = [System.Drawing.Color]::MediumSpringGreen
        Log "  ✔ Found Main Utility: $($Result.Version)" "Lime"
        $Script:AppMgrPath = Join-Path (Split-Path $Result.Path -Parent) "PnxAppMgr.exe"
    } else { 
        $lblVerDisplay.Text = "DB Ver: Not Found"; $lblVerDisplay.ForeColor = [System.Drawing.Color]::Salmon
...
# --- DEDICATED CODEBOOK SYNC FUNCTION ---
function Execute-CodebookSync {
    param($RawItems)
    try {
        if(-not $Script:CodebookRoot -or -not (Test-Path $Script:CodebookRoot)) {
            Log "  ⚠ Codebook Utility not installed. Skipping CB Sync." "Orange"; return
        }
        ...
        if ($fireTargets.Count -eq 0) {
            Log "  ⚠ No Fire databases selected. Skipping CB Sync." "Orange"; return
        }
...
"@
        ...
        if ($FailedDBs.Count -gt 0) {
            ...
            [System.Windows.Forms.MessageBox]::Show($msg, "Sync Completed with Errors", "OK", "Warning")
...
$Script:btnBackupDB.Add_Click({
    try {
        if($Script:listDBs.CheckedItems.Count -eq 0){ [System.Windows.Forms.MessageBox]::Show("Select Database(s) to Backup!", "Warning", "OK", "Warning"); return }
...
$Script:btnRestoreDB.Add_Click({
    try {
        if($Script:listDBs.CheckedItems.Count -ne 1){ [System.Windows.Forms.MessageBox]::Show("Select exactly ONE database to restore into.", "Warning", "OK", "Warning"); return }
        ...
        if([System.Windows.Forms.MessageBox]::Show("CRITICAL WARNING:`n`nYou are about to overwrite the existing database [$TargetDB] with:`n$BakFile`n`nAre you absolutely sure?", "Confirm RESTORE", "YesNo", "Error") -ne "Yes"){ return }
...
$Script:btnCon.Add_Click({
    try {
        ...
        # --- THE AUTO-SYNC PIPELINE FIX ---
        if ($Script:chkAutoSync.Checked -and $Script:AppMgrPath -and (Test-Path $Script:AppMgrPath)) {
            $ans = [System.Windows.Forms.MessageBox]::Show("Auto Update DB is checked.`n`nDo you also need to INSTALL and SYNC the Codebook Utility?", "Codebook Integration Pipeline", "YesNo", "Question")
            if ($ans -eq "Yes") { $Script:AutoCodebook = $true }
...
$RunAppMgr = { param($Mode)
    ...
    if ($Mode -eq "INSTALL") {
        ...
        $ans = [System.Windows.Forms.MessageBox]::Show("Do you also want to install the Codebook Utility (DatabaseUtilityCodebook)?", "Install Codebook?", "YesNo", "Question")
        if ($ans -eq "Yes") { $BatContent += "`"$($Script:AppMgrPath)`" INSTALL `"DatabaseUtilityCodebook`"`n" }
    } elseif ($Mode -eq "UNINSTALL") {
        $BatContent += "`"$($Script:AppMgrPath)`" UNINSTALL `"DBUtility`"`n"
        $ans = [System.Windows.Forms.MessageBox]::Show("Do you also want to uninstall the Codebook Utility?", "Uninstall Codebook?", "YesNo", "Question")
        if ($ans -eq "Yes") { $BatContent += "`"$($Script:AppMgrPath)`" UNINSTALL `"DatabaseUtilityCodebook`"`n" }
    }
    $BatContent += "pause"
