...
# ======================================================================
#  ProPhoenix DB Utility Dashboard v8.3
# ======================================================================
...
$Script:form.Text = "ProPhoenix DB Utility Dashboard v8.3"
...
# ======================================================================
# --- COPY DB (LIVE TO TRAIN/TEST) - PRO-MODE FAIL-SAFE LOGIC ---
# ======================================================================

function Show-CopyDialog {
    $d = New-Object System.Windows.Forms.Form
    $d.Text = "Select Copy Type"; $d.Size = New-Object System.Drawing.Size(300, 150); $d.StartPosition = "CenterParent"
    ...
    if ($d.ShowDialog($Script:form) -eq [System.Windows.Forms.DialogResult]::OK) { return $cb.SelectedItem } return $null
}
...
function Show-InputBox { 
    ...
    if ($f.ShowDialog($Script:form) -eq [System.Windows.Forms.DialogResult]::OK) { return $t.Text } return $null 
}

$Script:btnCopyDB.Add_Click({
    try {
        if($Script:listDBs.CheckedItems.Count -ne 1){ 
        ...
        $TypeSel = Show-CopyDialog
        if($null -eq $TypeSel){ return }
        ...
        # --- 1. SAFE DATABASE EXISTENCE CHECK (ICOMPARABLE & DBNULL FIX) ---
        $CS="Server=$($Script:txtS.Text);Database=master;User Id=$($Script:txtU.Text);Password=$($Script:txtP.Text);Connection Timeout=30"
        $CN=New-Object System.Data.SqlClient.SqlConnection($CS); $CN.Open()
        $Cmd=$CN.CreateCommand(); $Cmd.CommandTimeout = 0
        $Cmd.CommandText = "SELECT DB_ID('$TargetDB')"
        $targetExistsRaw = $Cmd.ExecuteScalar()

        # LOGIC CHANGE: Check for integer type to avoid mathematical comparison crashes with DBNull
        $targetExists = ($targetExistsRaw -is [int] -or $targetExistsRaw -is [long])

        if ($targetExists) {
            $ansBkp = [System.Windows.Forms.MessageBox]::Show("Target database [$TargetDB] already exists.`n`nBackup existing Params/Jobs before overwriting?", "Backup Existing Target?", [System.Windows.Forms.MessageBoxButtons]::YesNo, [System.Windows.Forms.MessageBoxIcon]::Question)
            if ($ansBkp -eq [System.Windows.Forms.DialogResult]::Yes) {
                
                $availableJuris = Get-DBJurisIDs -DBName $TargetDB
                $inputJuris = Show-JurisSelector "Source JurisID" "Select JurisID to backup from [$TargetDB]:" $availableJuris
                
                if (![string]::IsNullOrWhiteSpace($inputJuris)) {
                    ...
                    if($fbdBkp.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK){
                        Log "▶ BACKING UP EXISTING [$TargetDB]: JURIS $inputJuris..." "Cyan"
                        ...
                        $CNTarget=New-Object System.Data.SqlClient.SqlConnection("Server=$($Script:txtS.Text);Database=$TargetDB;User Id=$($Script:txtU.Text);Password=$($Script:txtP.Text);Connection Timeout=15"); $CNTarget.Open()
                        $cmdTarget = $CNTarget.CreateCommand()
                        $cmdTarget.CommandText = "SET NOCOUNT ON; SELECT ParamID, CAST(ParamValue AS NVARCHAR(MAX)) FROM Parameter WHERE ParamID IN ($($Script:TargetParams -join ',')) AND JurisID=$inputJuris"
                        $reader = $cmdTarget.ExecuteReader(); $BackupLines += "[PARAMETERS]"
                        
                        # LOGIC CHANGE: Null-safe extraction for parameters
                        while ($reader.Read()) { 
                            $pVal = if ($reader.IsDBNull(1)) { "" } else { $reader.GetString(1) }
                            $BackupLines += "$($reader.GetValue(0))=$pVal" 
                        }
                        $reader.Close(); $BackupLines += ""
                        ...
                        foreach ($job in $tempJobs) { 
                            ...
                            # LOGIC CHANGE: Null-safe extraction for job parameters
                            while ($pReader.Read()) { 
                        ...
                        $CNTarget.Close(); Log "  ✔ Pre-Overwrite Backup Saved." "Lime"
                    }
                }
            }
            if ([System.Windows.Forms.MessageBox]::Show("Confirm: DROP and REPLACE [$TargetDB]?", "Confirm Overwrite", [System.Windows.Forms.MessageBoxButtons]::YesNo, [System.Windows.Forms.MessageBoxIcon]::Error) -ne [System.Windows.Forms.DialogResult]::Yes) { $CN.Close(); return }
        }
        ...
        # LOGIC CHANGE: Null-shield for source DB size check
        $Cmd.CommandText="SELECT CAST(SUM(size)*8/1024 AS VARCHAR) FROM sys.master_files WHERE database_id=DB_ID('$SourceDB')"
        $SizeMBRaw = $Cmd.ExecuteScalar()
        $SizeMB = if ($null -eq $SizeMBRaw -or $SizeMBRaw -eq [System.DBNull]::Value) { "Unknown" } else { $SizeMBRaw.ToString() }
        ...
        Log "  > Backing up Source..."; [System.Windows.Forms.Application]::DoEvents()
        $Cmd.CommandText="BACKUP DATABASE [$SourceDB] TO DISK='$BakFile' WITH COPY_ONLY, INIT, FORMAT, COMPRESSION, STATS=10"
        $Cmd.ExecuteNonQuery()|Out-Null

        Log "  > Overwriting Target..."; [System.Windows.Forms.Application]::DoEvents()
        try { 
            $CmdDrop = New-Object System.Data.SqlClient.SqlCommand("IF EXISTS(SELECT 1 FROM sys.databases WHERE name='$TargetDB') BEGIN ALTER DATABASE [$TargetDB] SET SINGLE_USER WITH ROLLBACK IMMEDIATE; END", $CN)
            $CmdDrop.ExecuteNonQuery()|Out-Null 
        } catch { }
        ...
        $CN.Close(); Log "✔ Process Finished for [$TargetDB]." "Lime"; $Script:btnCon.PerformClick()
    } 
