# ======================================================================
#  ProPhoenix DB Utility Dashboard - v163.0 (True Native Logic Replica)
# ======================================================================
...
$form.Text = "ProPhoenix DB Utility Dashboard v163 (Native Replica Edition)"
...
$masterGrid.RowStyles.Add((New-Object System.Windows.Forms.RowStyle([System.Windows.Forms.SizeType]::Absolute, 160))) 
...
# --- ACTIONS ---
$pnlAct = New-Object System.Windows.Forms.TableLayoutPanel; $pnlAct.Dock="Fill"; $pnlAct.ColumnCount=3; $pnlAct.RowCount=3
...
$btnAdvDir = New-Object System.Windows.Forms.Button; $btnAdvDir.Text="📂 OPEN DB SYNC FOLDER"; $btnAdvDir.Dock="Fill"; $btnAdvDir.BackColor=[System.Drawing.Color]::FromArgb(60,60,60); $btnAdvDir.ForeColor="LightGray"; $btnAdvDir.FlatStyle="Flat"; $btnAdvDir.Font=$fontHeader; $pnlAct.Controls.Add($btnAdvDir, 0, 2)
$btnAdvKill = New-Object System.Windows.Forms.Button; $btnAdvKill.Text="⚡ KILL STRAY SQL SESSIONS"; $btnAdvKill.Dock="Fill"; $btnAdvKill.BackColor=[System.Drawing.Color]::FromArgb(60,60,60); $btnAdvKill.ForeColor="Orange"; $btnAdvKill.FlatStyle="Flat"; $btnAdvKill.Font=$fontHeader; $btnAdvKill.Margin=New-Object System.Windows.Forms.Padding(10,0,10,0); $pnlAct.Controls.Add($btnAdvKill, 1, 2)
$btnAdvInfo = New-Object System.Windows.Forms.Button; $btnAdvInfo.Text="ℹ️ ABOUT NATIVE SYNC (V163)"; $btnAdvInfo.Dock="Fill"; $btnAdvInfo.BackColor=[System.Drawing.Color]::FromArgb(60,60,60); $btnAdvInfo.ForeColor="LightSkyBlue"; $btnAdvInfo.FlatStyle="Flat"; $btnAdvInfo.Font=$fontHeader; $pnlAct.Controls.Add($btnAdvInfo, 2, 2)
...
function Toggle($s) { $btnCon.Enabled=$s; $btnSync.Enabled=$s; $btnBackup.Enabled=$s; $btnRestore.Enabled=$s; $btnVer.Enabled=$s; $btnCreateDB.Enabled=$s; $btnCopyDB.Enabled=$s; $btnDeleteDB.Enabled=$s; $btnAdvKill.Enabled=$s; }
...
$btnAdvDir.Add_Click({
    if ($Script:DBSyncRoot -and (Test-Path $Script:DBSyncRoot)) { Start-Process "explorer.exe" $Script:DBSyncRoot }
    else { Log "Path not found." "Red" }
})

$btnAdvKill.Add_Click({
    if($listDBs.CheckedItems.Count -eq 0){ [System.Windows.Forms.MessageBox]::Show("Select DBs to Kill Sessions!"); return }
    Log "Killing active sessions on selected databases..." "Orange"
    try {
        ...
        foreach($i in $listDBs.CheckedItems){
            ...
            Log "   ⚡ Terminated sessions for $DB" "Lime"
        }
        $CN.Close()
    } catch { Log "❌ Kill Failed: $($_.Exception.Message)" "Red" }
})

$btnAdvInfo.Add_Click({
    [System.Windows.Forms.MessageBox]::Show("ProPhoenix DB Utility Dashboard v163`n`nFeatures:`n- Relies entirely on native Exit Code (No false failures)`n- Mirrors manual double-click execution exactly`n- Safe Infinite Timeouts applied to large SQL copies", "About", "OK", "Information")
})

# --- REUSABLE SYNC FUNCTION (TRUE NATIVE REPLICA) ---
function Execute-DBSync {
    ...
    foreach ($i in $Items) {
        ...
        try {
            ...
            # EXECUTE SYNC - Wait for it to close natively
            $SyncProc = Start-Process "$WD\PnxDBSync.exe" -WorkingDirectory $WD -WindowStyle Minimized -PassThru -Wait
            
            # NO LOG PARSING. Relies 100% on the native Exit Code.
