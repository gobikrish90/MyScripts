...
# ======================================================================
# --- COPY DB (LIVE TO TRAIN/TEST) - DBNULL ICOMPARABLE FIX ---
# ======================================================================
...
$Script:btnCopyDB.Add_Click({
    try {
        ...
        # --- 1. PRO-MODE FAIL-SAFE EXISTENCE CHECK ---
        $CS="Server=$($Script:txtS.Text);Database=master;User Id=$($Script:txtU.Text);Password=$($Script:txtP.Text);Connection Timeout=30"
        $CN=New-Object System.Data.SqlClient.SqlConnection($CS); $CN.Open()
        $Cmd=$CN.CreateCommand(); $Cmd.CommandTimeout = 0
        ...
        # THE FIX: Explicitly check for DBNull type to completely bypass IComparable crash
        $targetExists = $false
        if ($null -ne $targetExistsRaw -and -not ($targetExistsRaw -is [System.DBNull])) {
            if (([int]$targetExistsRaw) -gt 0) {
                $targetExists = $true
            }
        }

        if ($targetExists) {
            $ansBkp = [System.Windows.Forms.MessageBox]::Show("Target database [$TargetDB] already exists.`n`nDo you want to backup its current Parameters/Jobs before overwriting?", "Existing Target Detected", [System.Windows.Forms.MessageBoxButtons]::YesNo, [System.Windows.Forms.MessageBoxIcon]::Question)
            
            if ($ansBkp -eq [System.Windows.Forms.DialogResult]::Yes) {
                $availableJuris = Get-DBJurisIDs -DBName $TargetDB
                $selJ = Show-JurisSelector "Backup Source Juris" "Select JurisID to backup from existing [$TargetDB]:" $availableJuris
                ...
                if (![string]::IsNullOrWhiteSpace($selJ)) {
                    ...
                    if($fbdBkp.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK){
                        ...
                        $BackupLines = @("# ProPhoenix Pre-Overwrite Backup", "# Database: $TargetDB", "# JurisID: $selJ", "")
                        ...
                        $cmdTarget.CommandText = "SELECT ParamID, CAST(ParamValue AS NVARCHAR(MAX)) FROM Parameter WHERE ParamID IN ($($Script:TargetParams -join ',')) AND JurisID=$selJ"
                        ...
                        foreach ($job in $tempJobs) { 
                            ...
                            $pReader = $cmdTarget.ExecuteReader(); while ($pReader.Read()) { $BackupLines += "Param:$($pReader.IsDBNull(0)?'':$pReader.GetString(0))=$($pReader.IsDBNull(1)?'':$pReader.GetString(1))" }; $pReader.Close()
                            $cmdTarget.CommandText = "SELECT * FROM KPIJobsNotify WHERE JobID = $($job.ID)"; $nReader = $cmdTarget.ExecuteReader(); while ($nReader.Read()) { if (!($nReader.IsDBNull(4))) { $BackupLines += "NotifyPath=$($nReader.GetString(4))" } }; $nReader.Close(); $BackupLines += "" 
                        }
                        [IO.File]::WriteAllLines((Join-Path $fbdBkp.SelectedPath "$($TargetDB)_SafetyBackup_$TS.txt"), $BackupLines, [System.Text.Encoding]::UTF8)
                        $CNTarget.Close(); Log "  ✔ Safety Backup Saved." "Lime"
                    }
                }
            }
            if ([System.Windows.Forms.MessageBox]::Show("Final Confirmation: Drop and Overwrite [$TargetDB]?", "WARNING", [System.Windows.Forms.MessageBoxButtons]::YesNo, [System.Windows.Forms.MessageBoxIcon]::Stop) -ne [System.Windows.Forms.DialogResult]::Yes) { $CN.Close(); return }
        }

        # --- 2. POST-COPY RESTORE PROMPT ---
        $ansRest = [System.Windows.Forms.MessageBox]::Show("Apply a configuration backup (.txt) to [$TargetDB] after copy finishes?", "Post-Copy Restore", [System.Windows.Forms.MessageBoxButtons]::YesNo, [System.Windows.Forms.MessageBoxIcon]::Question)
        ...
        if ($ansRest -eq [System.Windows.Forms.DialogResult]::Yes) {
            ...
            if ($f.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) { 
                ...
                $TargetJurisID = Show-InputBox "Target JurisID" "Enter JurisID for restoration into [$TargetDB]:" "1000"
                if ([string]::IsNullOrWhiteSpace($TargetJurisID)) { $CN.Close(); return }
            }
        }

        # --- 3. DATABASE COPY OPERATION ---
        Toggle $false
        $CN.FireInfoMessageEventOnUserErrors = $true
        $CN.add_InfoMessage({ param($sender, $e) if ($e.Message -match "processed") { Log "   > $($e.Message)" "DarkGray" } })
        ...
        # THE FIX: Safely check for DBNull on SizeMB query as well
        $SizeMB = "Unknown"
        if ($null -ne $SizeMBRaw -and -not ($SizeMBRaw -is [System.DBNull])) {
            $SizeMB = $SizeMBRaw.ToString()
        }
        
        if([System.Windows.Forms.MessageBox]::Show("Begin migration of $SourceDB ($SizeMB MB)?", "Confirm Copy", [System.Windows.Forms.MessageBoxButtons]::YesNo) -ne [System.Windows.Forms.DialogResult]::Yes){ $CN.Close(); return }
        ...
        $BakFile = Join-Path $fbdSource.SelectedPath "$SourceDB`_Migration.bak"
        Log "▶ EXECUTING MIGRATION: $SourceDB -> $TargetDB" "Cyan"
        
        Log "  > Creating source backup..."; [System.Windows.Forms.Application]::DoEvents()
        $Cmd.CommandText="BACKUP DATABASE [$SourceDB] TO DISK='$BakFile' WITH COPY_ONLY, INIT, FORMAT, COMPRESSION, STATS=10"
        $Cmd.ExecuteNonQuery()|Out-Null

        Log "  > Forcing single user on target..."; [System.Windows.Forms.Application]::DoEvents()
        try { 
        ...
        Log "  > Restoring and mapping files..."; [System.Windows.Forms.Application]::DoEvents()
        $Cmd.CommandText="RESTORE FILELISTONLY FROM DISK='$BakFile'"; $Rdr=$Cmd.ExecuteReader(); $Files=@(); while($Rdr.Read()){$Files+=@{L=$Rdr["LogicalName"];P=$Rdr["PhysicalName"];T=$Rdr["Type"]}}; $Rdr.Close()
        ...
        if (![string]::IsNullOrEmpty($ConfigFilePath)) {
            Log "▶ APPLYING RESTORE: Juris $TargetJurisID..." "Yellow"
            Apply-EnterpriseConfig -TargetDB $TargetDB -SourceFile $ConfigFilePath -TargetJuris $TargetJurisID
        }

        $CN.Close(); Log "✔ Full deployment for [$TargetDB] successful." "Lime"; $Script:btnCon.PerformClick()
    } 
    catch { Log "❌ DEPLOYMENT FAILED: $($_.Exception.Message)" "Red" } 
    finally { Toggle $true }
})
