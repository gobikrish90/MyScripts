# ======================================================================
#  ProPhoenix Sync Manager - v42.0 (Universal Remote/Local Scanner)
# ======================================================================
...
$Script:WindowsCreds = $null # Stores Remote Admin Credentials
...
function Scan-Server {
    ...
    # Execute Scan (Local or Remote)
    try {
        if ($Script:IsRemote) {
            try {
                if ($Script:WindowsCreds) { $Data = Invoke-Command -ComputerName $Target -Credential $Script:WindowsCreds -ScriptBlock $Block -ErrorAction Stop }
                else { $Data = Invoke-Command -ComputerName $Target -ScriptBlock $Block -ErrorAction Stop }
            } catch {
                if ($_.Exception.Message -match "Access is denied" -or $_.Exception.Message -match "authorization") {
                    Log-Write "⚠ Access Denied. Prompting for Admin Creds..." "Yellow"
                    $Script:WindowsCreds = $host.ui.PromptForCredential("Remote Admin Required", "Enter Admin Creds for $Target", "$Target\Administrator", "")
                    if ($Script:WindowsCreds) {
                        $Data = Invoke-Command -ComputerName $Target -Credential $Script:WindowsCreds -ScriptBlock $Block -ErrorAction Stop
...
$btnSync.Add_Click({
    ...
    foreach ($db in $listDBs.CheckedItems) {
        ...
        try {
            ...
            if ($logData -match "DB Version Updated") { Log-Write "   ✔ Success [ $finalTime ]" "Lime" } 
        ...
        $lblStat.Text = "Completed: $db ( $newPct % )"
        [System.Windows.Forms.Application]::DoEvents()
    }
...
function Exec-Util($act) {
    ...
    try {
        ...
        # Auto-Refresh with same logic as scan
