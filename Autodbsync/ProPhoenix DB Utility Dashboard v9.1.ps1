...
# ======================================================================
#  ProPhoenix DB Utility Dashboard v9.1
# ======================================================================
...
$Script:form.Text = "ProPhoenix DB Utility Dashboard v9.1"
...
# ======================================================================
# --- COPY DB (LIVE TO TRAIN/TEST) - GETSTRING() CRASH FIX ---
# ======================================================================
            ...
            if ($ansBkp.Equals([System.Windows.Forms.DialogResult]::Yes)) {
                ...
                if (![string]::IsNullOrWhiteSpace($selJ)) {
                    ...
                    if($fbdBkp.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK){
                        ...
                        # LOGIC CHANGE: Replaced GetString() with GetValue() to avoid strict null crashes
                        while ($reader.Read()) { 
                            $v1 = $reader.GetValue(1)
                            $pVal = if ($v1 -is [System.DBNull] -or $null -eq $v1) { "" } else { $v1.ToString() }
                        ...
                        foreach ($job in $tempJobs) { 
                            ...
                            while ($nReader.Read()) { 
                                $v4 = $nReader.GetValue(4)
                                if ($v4 -isnot [System.DBNull] -and $null -ne $v4) { $BackupLines += "NotifyPath=$($v4.ToString())" } 
                            }
...
$Script:btnBackup.Add_Click({
    try {
        ...
        foreach ($item in $Script:listDBs.CheckedItems) {
            $DB = $Script:TargetMap[$item].DB; Log "  > Backing up Config for $DB..."; 
            ...
            # LOGIC FIX: Replaced GetString with GetValue & DBNull shield for Parameters
            while ($reader.Read()) { 
                $v1 = $reader.GetValue(1)
                $pVal = if ($v1 -is [System.DBNull] -or $null -eq $v1) { "" } else { $v1.ToString() }
                $BackupLines += "$($reader.GetValue(0))=$pVal"
            ...
            $reader.Close(); Log "    + Exported $paramCount Parameters"
            $BackupLines += ""
            ...
            # LOGIC FIX: Replaced GetString with GetValue & DBNull shield for Jobs
            while ($jobReader.Read()) { 
                $v1 = $jobReader.GetValue(1); $jName = if ($v1 -is [System.DBNull]) { "Unknown" } else { $v1.ToString() }
                $v2 = $jobReader.GetValue(2); $jType = if ($v2 -is [System.DBNull]) { "" } else { $v2.ToString() }
                $tempJobs += @{ ID = $jobReader.GetValue(0); Name = $jName; Type = $jType } 
            ...
            foreach ($job in $tempJobs) { 
                ...
                # LOGIC FIX: Replaced GetString with GetValue & DBNull shield for Job Parameters
                while ($pReader.Read()) { 
                    $v0 = $pReader.GetValue(0); $jpName = if ($v0 -is [System.DBNull]) { "" } else { $v0.ToString() }
                    $v1 = $pReader.GetValue(1); $jpVal = if ($v1 -is [System.DBNull]) { "" } else { $v1.ToString() }
                    $BackupLines += "Param:$jpName=$jpVal" 
                ...
                # LOGIC FIX: Replaced GetString with GetValue & DBNull shield for Notify Paths
                while ($nReader.Read()) { 
                    $v4 = $nReader.GetValue(4)
                    if ($v4 -isnot [System.DBNull] -and $null -ne $v4) { $BackupLines += "NotifyPath=$($v4.ToString())" } 
                }
