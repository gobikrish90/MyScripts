# ======================================================================
#  ProPhoenix Sync Manager - v19.0 (Sync + DB Utility Maintenance)
# ======================================================================
...
$form.Size = New-Object System.Drawing.Size(960, 800) # Increased Height
...
$grpUtil.Text = " DB Utility Maintenance "; $grpUtil.Location = "10,610"; $grpUtil.Size = "920,80"; $grpUtil.ForeColor = "Magenta"
...
$btnVer.Add_Click({
    ...
    finally { Toggle-Inputs $true; $btnStop.Enabled = $false; $lblStat.Text = "Ready." }
})
...
$RunAppMgrAction = {
    ...
    if (Test-Path $ExePath) {
        Start-Process -FilePath $ExePath -ArgumentList "$Action", "DBUtility" -Wait -WindowStyle Hidden
        return "SUCCESS: Executed $Action for DBUtility"
    } else { return "ERROR: PnxAppMgr.exe not found at $AppMgrFolder" }
}

function Exec-Utility($actionName) {
    Toggle-Inputs $false
    $lblStat.Text = "$actionName DB Utility..."
    Log-Write "Starting $actionName of DB Utility..." "Cyan"
    ...
    try {
        if ($IsLocal) {
            $res = & $RunAppMgrAction -Action $actionName
            ...
            $res = Invoke-Command -Session $s -ScriptBlock $RunAppMgrAction -ArgumentList $actionName
            Remove-PSSession $s
            if ($res -match "SUCCESS") { Log-Write $res "Lime" } else { Log-Write $res "Red" }
        }
    } catch {
        ...
        Toggle-Inputs $true
        $lblStat.Text = "Ready."
        [System.Windows.Forms.MessageBox]::Show("$actionName Process Finished.", "Done")
    }
}
