# ======================================================================
#  ProPhoenix Sync Manager - v23.1 (Percentage Format & Full Logs)
# ======================================================================
...
$pbStat = New-Object System.Windows.Forms.ToolStripProgressBar; $pbStat.Size="200,16"; $statusStrip.Items.Add($pbStat)
...
$btnSync.Add_Click({
    ...
    Toggle-Inputs $false
    $count = $listDBs.CheckedItems.Count
    $current = 0
    $pbStat.Maximum = $count
    $pbStat.Value = 0
    ...
    foreach ($db in $listDBs.CheckedItems) {
        ...
        # --- UPDATE STATUS & LOG WITH PERCENTAGE ---
        $lblStat.Text = "Processing: $db ... $percent %"
        $pbStat.Value = $current
        Log-Write "[ $percent % ] Processing $db..." "Cyan"
    ...
    $lblStat.Text = "Sync Cycle Finished."; Toggle-Inputs $true; [System.Windows.Forms.MessageBox]::Show("Sync Complete", "Done")
})
...
$RunAppMgrAction = {
    ...
    Set-Content $BatFile "@echo off`ncd /d `"$((Split-Path $AppRegPath -Parent))`"`n`"$Exe`" `"$Action`" `"DBUtility`"`npause" -Encoding ASCII
