<#
.SYNOPSIS
    ProPhoenix Master Launcher (v8 - Layout & Syntax Fix)
    - Fixed "New-Object Point" overload error by pre-calculating coordinates.
    - Increased Window Size to 600x750 for future expansions.
    - Auto-deploys tools to C:\pnxtemp\ProPhoenixSuite
    - Logging enabled at C:\ProPhoenix_AutoTool\Logs
#>

# ==============================================================================
#  1. GLOBAL SETTINGS & LOGGING
# ==============================================================================
$LogBaseDir = "C:\ProPhoenix_AutoTool\Logs"
if (-not (Test-Path $LogBaseDir)) { New-Item -ItemType Directory -Path $LogBaseDir -Force | Out-Null }
$GlobalLogFile = Join-Path $LogBaseDir "MasterLauncher_Log_$(Get-Date -Format 'yyyyMMdd').txt"

function Write-Log {
    param([string]$Message, [string]$Level="INFO")
    $TimeStamp = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
    $LogEntry = "[$TimeStamp] [$Level] $Message"
    try { Add-Content -Path $GlobalLogFile -Value $LogEntry -ErrorAction SilentlyContinue } catch {}
...
# ==============================================================================
#  3. DEFINE EMBEDDED SCRIPTS
# ==============================================================================

# --- SCRIPT 1: Demo / Test ---
$Content_DemoTest = @'
<# .SYNOPSIS: ProPhoenix Hotfix Automation (Demo/Test) #>
Clear-Host
...
foreach (`$folder in `$instanceFolders) { foreach (`$environmentType in `$environmentTypes) { `$targetPath = "`$(`$folder.FullName)\_Instances\`$environmentType\PnxLog"; `$oldFolderPath = "`$targetPath\old"; if (Test-Path -Path `$targetPath) { `$filesToMove = Get-ChildItem -Path `$targetPath -File | Where-Object { `$_.FullName -notlike "`$oldFolderPath*" }; if (`$filesToMove.Count -gt 0) { if (!(Test-Path -Path `$oldFolderPath)) { New-Item -ItemType Directory -Path `$oldFolderPath | Out-Null }; Get-ChildItem -Path `$oldFolderPath -File | Remove-Item -Force; foreach (`$file in `$filesToMove) { Move-Item -Path `$file.FullName -Destination `$oldFolderPath }; Write-Host "[SUCCESS] Logs processed for: `$environmentType (`$targetPath)" } else { Write-Host "[INFO] No log files in: `$targetPath. Skipping..." } } } }
Write-Host "`n========================================="; Write-Host "       CLEARING SERVER SESSION DATA      "; Write-Host "========================================="
foreach (`$basePath in `$proPhoenixBasePaths) { `$cadServerPath = Join-Path -Path `$basePath -ChildPath "CAD Server\_Instances"; if (Test-Path `$cadServerPath) { `$cadInstances = Get-ChildItem -Path `$cadServerPath -Directory; foreach (`$inst in `$cadInstances) { `$sessionDataPath = Join-Path -Path `$inst.FullName -ChildPath "SvrSessionData"; if (Test-Path `$sessionDataPath) { Write-Host "[INFO] Cleaning Session Data: `$sessionDataPath"; `$filesDeleted = 0; Get-ChildItem -Path `$sessionDataPath -File | ForEach-Object { Remove-Item -Path `$_.FullName -Force; `$filesDeleted++ }; if (`$filesDeleted -gt 0) { Write-Host "  [SUCCESS] Removed `$filesDeleted session files." } } } } }
Write-Host "`n========================================="; Write-Host "             SCRIPT COMPLETED            "; Write-Host "========================================="
"@
    $VerificationScriptContent = @"
[Console]::OutputEncoding = [System.Text.Encoding]::ASCII
function Log-Msg { param([string]`$Msg, [ConsoleColor]`$Color = "White"); Write-Host `$Msg -ForegroundColor `$Color; if (`$env:LOGFILE) { for (`$i=0; `$i -lt 5; `$i++) { try { Add-Content -Path `$env:LOGFILE -Value `$Msg -Encoding ASCII -ErrorAction Stop; break } catch { Start-Sleep -Milliseconds 100 } } } }
Log-Msg "[INFO] Starting DLL verification..." "Cyan"
`$prophoenixPaths = Get-PSDrive -PSProvider FileSystem | ForEach-Object { `$driveRoot = "`$(`$_.Root)Program Files\ProPhoenix"; if (Test-Path `$driveRoot) { `$driveRoot } }
if (-not `$prophoenixPaths) { Log-Msg "[ERROR] No ProPhoenix installation found." "Red"; exit }
Log-Msg "`n[INFO] Found base paths:" "Cyan"; `$prophoenixPaths | ForEach-Object { Log-Msg " - `$_" "Gray" }
`$excludedFolders = @("Finger Print Client","ID Scanner","Phoenix WDA V2","Police RMS","PoliceRMS","Print Server","WDA"); `$completedCount = 0; `$notCompletedCount = 0; `$totalChecked = 0
foreach (`$basePath in `$prophoenixPaths) {
    ...
    foreach (`$app in `$appFolders) {
        `$instancesPath = Join-Path `$app.FullName "_Instances"; if (Test-Path `$instancesPath) {
            `$instanceEnvs = Get-ChildItem -Path `$instancesPath -Directory -ErrorAction SilentlyContinue
            foreach (`$env in `$instanceEnvs) {
                `$baseDlls = Get-ChildItem -Path `$app.FullName -Filter *.dll -File -ErrorAction SilentlyContinue; `$instanceDlls = Get-ChildItem -Path `$env.FullName -Filter *.dll -File -ErrorAction SilentlyContinue; `$status = "Completed"
                ...
                `$totalChecked++; if (`$status -eq "Completed") { `$completedCount++; Log-Msg ("[OK] " + `$app.Name + " -> " + `$env.Name + " : Completed") "Green" } else { `$notCompletedCount++; Log-Msg ("[WARN] " + `$app.Name + " -> " + `$env.Name + " : Not Completed") "Red" }
            }
        }
    }
}
Log-Msg "`n[INFO] DLL verification completed." "Cyan"; Log-Msg "--------------------------------------" "White"; Log-Msg ("Total Verified : " + `$totalChecked) "White"; Log-Msg ("Completed      : " + `$completedCount) "Green"; Log-Msg ("Not Completed  : " + `$notCompletedCount) "Red"; Log-Msg "--------------------------------------" "White"
"@
...
if ($RunRemote) {
    ...
    try { Invoke-Command -ComputerName $TargetServer -Credential $Creds -ScriptBlock $GeneratorLogic; Write-Host "`n[INFO] Files generated on REMOTE server ($TargetServer)." -ForegroundColor Yellow; Write-Host "       You must log in to the remote server to run them." } catch { Write-Error "Connection failed. Please check Server Name and Credentials."; Write-Error $_ }
} else {
...
# --- SCRIPT 3: PD CAD ---
$Content_CAD = @'
<# .SYNOPSIS: ProPhoenix Hotfix Automation (Production / Installation Team). #>
Clear-Host
Write-Host "==============================================" -ForegroundColor Cyan
    ...
    foreach (`$instRoot in `$instanceRoots) {
        ...
        foreach (`$env in `$environments) {
            ...
            if (Test-Path `$logPath) {
                Write-Host "------------------------------------------------"
                Write-Host " INSTANCE: `$(`$env.Name)"; Write-Host " LOCATION: `$logPath"; Write-Host "------------------------------------------------"
                `$oldPath = Join-Path -Path `$logPath -ChildPath "old"; if (!(Test-Path `$oldPath)) { New-Item -ItemType Directory -Path `$oldPath -Force | Out-Null }
                ...
                `$movedCount = 0; Get-ChildItem -Path `$logPath -File -ErrorAction SilentlyContinue | Where-Object { `$_.FullName -notlike "*\old*" } | ForEach-Object { try { Move-Item -LiteralPath `$_.FullName -Destination `$oldPath -Force -ErrorAction Stop; Write-Host "   [MOVED] `$(`$_.Name) -> old"; `$movedCount++ } catch { Write-Host "   [LOCKED] `$(`$_.Name) (Skipped)" -ForegroundColor DarkGray } }
    ...
    if (Test-Path `$cadServerPath) {
        ...
        foreach (`$inst in `$cadInstances) {
            ...
            if (Test-Path `$sessionDataPath) {
                Write-Host "------------------------------------------------"
                Write-Host " SESSION DATA: `$(`$inst.Name)"; Write-Host " LOCATION:     `$sessionDataPath"; Write-Host "------------------------------------------------"
                `$delCount = 0; Get-ChildItem -Path `$sessionDataPath -File -ErrorAction SilentlyContinue | ForEach-Object { try { Remove-Item -Path `$_.FullName -Force -ErrorAction Stop; Write-Host "   [DELETED] `$(`$_.Name)"; `$delCount++ } catch { Write-Host "   [LOCKED] `$(`$_.Name)" -ForegroundColor DarkGray } }
...
Log-Msg "`n--------------------------------------" "White"; Log-Msg ("Total Verified : " + (`$completedCount + `$notCompletedCount)) "White"; Log-Msg ("Completed      : " + `$completedCount) "Green"; Log-Msg ("Not Completed  : " + `$notCompletedCount) "Red"; Log-Msg "--------------------------------------" "White"
"@
    $ClientAppsDef = @{ "CAD Client"="KPI.Phoenix.CADClient.exe"; "WDA"="KPI.Phoenix.CADMobileClient.exe"; "Phoenix WDA V2"="Phoenix.WDAV2.Client.Shell.exe" }
    $ServiceProductList = @("JobServer", "TraCSServer", "VideoServer", "FingerPrintServer", "EmailWatcher", "CADServer", "CADNLBServer", "CAD2CADTellusServer", "E911Server", "ZetronServer", "ExternalInterface", "GPSServer", "NCICServer", "NCICStateServer", "FTPServer", "LocutionCADVoiceServer", "DeviceNotification", "StreamingNotification", "ReportService", "FolderWatcher", "DocsServer", "PhoenixTonerServer", "PhoenixAlertApp", "PhoenixTExt2Dispatch", "PHOENIXAIWATCHERSERVICE", "PHOENIXJOBSVRV2")
    $RawMappingData = @"
...
# ==============================================================================
#  4. DEPLOYMENT & EXECUTION LOGIC
# ==============================================================================
$SuitePath = "C:\pnxtemp\ProPhoenixSuite"
Write-Log "Checking Suite Folder: $SuitePath"
if (-not (Test-Path $SuitePath)) { 
    New-Item -ItemType Directory -Path $SuitePath -Force | Out-Null 
    Write-Log "Created Folder: $SuitePath" "INFO"
}
...
Write-Log "Deploying Embedded Scripts..."
...
# ==============================================================================
#  5. GUI LAUNCHER (CUSTOMIZED - NO LOGO)
# ==============================================================================
Add-Type -AssemblyName System.Windows.Forms
Add-Type -AssemblyName System.Drawing

# --- SETTINGS ---
$AppTitle = "ProPhoenix Installation Launcher"

$form = New-Object System.Windows.Forms.Form
$form.Text = $AppTitle
$form.Size = New-Object System.Drawing.Size(600, 750) # INCREASED HEIGHT to fix cutoff
...
$BtnWidth = 450
$BtnSpacing = 30
$CenterX = ($form.Size.Width - $BtnWidth) / 2 - 10 # Centering calculation
...
$lblTitle.Location = New-Object System.Drawing.Point(0, $TitleTop)
$lblTitle.Size = New-Object System.Drawing.Size($form.Size.Width, 50)
...
# --- PRE-CALCULATE POSITIONS TO FIX "POINT" ERROR ---
$Pos_Y1 = $ButtonTopStart
$Pos_Y2 = $ButtonTopStart + $BtnHeight + $BtnSpacing
$Pos_Y3 = $ButtonTopStart + ($BtnHeight + $BtnSpacing) * 2
$Pos_Status = $Pos_Y3 + $BtnHeight + 60
$Pos_Path = $Pos_Status + 30
$Pos_Log = $Pos_Path + 30

# --- BUTTON 1: DEMO / TEST ---
$btnDemo = New-Object System.Windows.Forms.Button
$btnDemo.Text = "Hotfix Update - Demo / Test"
$btnDemo.Location = New-Object System.Drawing.Point($CenterX, $Pos_Y1)
$btnDemo.Size = New-Object System.Drawing.Size($BtnWidth, $BtnHeight)
$btnDemo.BackColor = [System.Drawing.Color]::FromArgb(255, 140, 0) # Orange
...
$btnSync.Location = New-Object System.Drawing.Point($CenterX, $Pos_Y2)
$btnSync.Size = New-Object System.Drawing.Size($BtnWidth, $BtnHeight)
...
$btnCAD.Location = New-Object System.Drawing.Point($CenterX, $Pos_Y3)
$btnCAD.Size = New-Object System.Drawing.Size($BtnWidth, $BtnHeight)
$btnCAD.BackColor = [System.Drawing.Color]::FromArgb(220, 20, 60) # Red
...
$lblStatus.Location = New-Object System.Drawing.Point(0, $Pos_Status)
$lblStatus.Size = New-Object System.Drawing.Size($form.Size.Width, 30)
...
$lblPath.Text = "Tools: $SuitePath"
$lblPath.Location = New-Object System.Drawing.Point(0, $Pos_Path)
$lblPath.Size = New-Object System.Drawing.Size($form.Size.Width, 20)
$lblPath.TextAlign = "MiddleCenter"
$lblPath.ForeColor = "DarkGray"
$form.Controls.Add($lblPath)

$lblLog = New-Object System.Windows.Forms.Label
$lblLog.Text = "Logs: $LogBaseDir"
$lblLog.Location = New-Object System.Drawing.Point(0, $Pos_Log)
$lblLog.Size = New-Object System.Drawing.Size($form.Size.Width, 20)
$lblLog.TextAlign = "MiddleCenter"
$lblLog.ForeColor = "Yellow"
$form.Controls.Add($lblLog)

# --- FUNCTION TO LAUNCH ---
function Launch-Script($scriptName, $fullPath) {
    if (Test-Path $fullPath) {
        $msg = "Launching Tool: $scriptName"
        $lblStatus.Text = $msg
        $lblStatus.ForeColor = "Yellow"
        Write-Log $msg "ACTION"
        [System.Windows.Forms.Application]::DoEvents()
        
        try {
            Start-Process -FilePath "powershell.exe" -ArgumentList "-ExecutionPolicy Bypass -File `"$fullPath`"" -Verb RunAs
            Write-Log "Tool launched successfully." "SUCCESS"
        } catch {
            Write-Log "Failed to launch tool: $_" "ERROR"
        }
        ...
        $err = "Script not found at $fullPath"
        Write-Log $err "ERROR"
        [System.Windows.Forms.MessageBox]::Show($err, "Missing File", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
    }
}
...
# --- SHOW FORM ---
$form.Add_Shown({ $form.Activate(); Write-Log "GUI Displayed." })
[void] $form.ShowDialog()
Write-Log "Launcher Closed."