<#
.SYNOPSIS
    ProPhoenix Full Maintenance Generator (INTERACTIVE & HOSTNAME AWARE).
    
    FEATURES:
    1. Interactive Prompt: Asks for Server Name (Local or Remote).
    2. Auto-Naming: Generates "HOSTNAME_Hotfix_Automation.bat".
    3. Hybrid Logic: Runs exactly the same code locally or remotely via WinRM.
#>
...
Write-Host "   PROPHOENIX MAINTENANCE GENERATOR SETUP" -ForegroundColor Cyan
Write-Host "==============================================" -ForegroundColor Cyan
...
%PSExe% -Command "Get-Service -Name 'Phoenix*' | Where-Object { `$_.Status -eq 'Stopped' } | ForEach-Object { Start-Service -Name `$_.Name; Write-Host 'Started ' `$_.Name }" *>&1 | %PSExe% -Command "`$input | ForEach-Object { Write-Host `$_; `$_ | Out-File -FilePath '%LOGFILE%' -Append -Encoding ASCII }" 2>&1
...
%PSExe% -ExecutionPolicy Bypass -File "%~dp0InstanceVerification.ps1" *>&1 | %PSExe% -Command "`$input | ForEach-Object { Write-Host `$_; `$_ | Out-File -FilePath '%LOGFILE%' -Append -Encoding ASCII }" 2>&1
...
"@
    ...
    Write-Host "`n[DONE] Finished processing on $CurrentHostName" -ForegroundColor Yellow
}
