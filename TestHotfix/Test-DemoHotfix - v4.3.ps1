<#
.SYNOPSIS
    ProPhoenix Full Maintenance Generator (HELP DOCS FIX).
    1. VERIFICATION UPDATE: Now detects and verifies nested 'UserDocs' folders (Police/Fire/IA).
    2. INCLUDES: All previous features (Pause, Red Text Fix, Interactive Launch, Full Mappings).
#>
...
# (B) Instance Verification Script (UPDATED: DETECTS NESTED HELP DOCS)
...
Log-Msg "[INFO] Starting verification..." "Cyan"
...
foreach (`$basePath in `$prophoenixPaths) {
    ...
    foreach (`$app in `$appFolders) {
        ...
        # Look for 'UserDocs' subfolder inside this app
...
foreach ($FoundPath in $FoundFiles) {
    ...
    # --- BATCH FILE CONTENT ---
    # FIX: "UPDAPPMANAGER" is now at the very top.
    # FIX: Hub/Gateways excluded. Verification paused.
    $BatchContent = @"
$CdBlock
@echo off
