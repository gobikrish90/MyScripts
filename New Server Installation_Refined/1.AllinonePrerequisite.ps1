# ==============================================================================
# ProPhoenix New Server Pre - requisites
# Version: 3.2
# ==============================================================================
...
Function Install-Package {
    ...
    # Registry Check for Idempotency
    if ($UninstallDisplayName) {
        $uninstallPaths = @(
            "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\*",
            "HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\*"
        )
        $foundSoftware = Get-ItemProperty -Path $uninstallPaths -ErrorAction SilentlyContinue | Where-Object { $_.DisplayName -like "*$UninstallDisplayName*" }
        if ($foundSoftware) {
    ...
    try {
        ...
        if ($process.ExitCode -in @(0, 3010)) {
            ...
            $details = if ($process.ExitCode -eq 3010) { "Reboot Required" } else { "Success" }
            Record-TaskStatus -TaskName "Install: $SoftwareName" -Status "Success" -Details $details
        ...
        Write-Log "Failed to process $SoftwareName : $($_.Exception.Message)" -Level ERROR
...
Install-Package -Url "https://produpdates.blob.core.windows.net/web/Pre-requsite%20Application-%202026/Prerequsite%20softwares/Phoenix%20Server%20Application%20Manager%202024%20-%20FIPS.zip?sp=racw&st=2026-02-27T09:27:17Z&se=2031-02-27T17:42:17Z&spr=https&sv=2024-11-04&sr=b&sig=GEfnpeBEBgmWhaetS14kgaDbdWhIgSLUC%2FtbzmhjLA8%3D" -DestinationPath "$phoenixAppManagerTempPath\PhoenixServerApplicationManager.zip" -SoftwareName "Phoenix Server App Manager" -UninstallDisplayName "Phoenix Server Application Manager" -InstallDirectory "${phoenixAppManagerInstallDrive}:\Program Files (x86)\ProPhoenix\Phoenix Server Application Manager"
Install-Package -Url "https://produpdates.blob.core.windows.net/web/Pre-requsite%20Application-%202026/Prerequsite%20softwares/Phoenix%20Client%20Application%20Manager%202024%20FIPS.zip?sp=racw&st=2026-02-27T09:26:33Z&se=2031-02-27T17:41:33Z&spr=https&sv=2024-11-04&sr=b&sig=FxAf6Vwa0EHcwQrUGw5msSk5dwwN6z79emPPxe0ioik%3D" -DestinationPath "$phoenixAppManagerTempPath\PhoenixClientApplicationManager.zip" -SoftwareName "Phoenix Client App Manager" -UninstallDisplayName "Phoenix Client Application Manager" -InstallDirectory "${phoenixAppManagerInstallDrive}:\Program Files (x86)\ProPhoenix\Phoenix Client Application Manager"
Install-Package -Url "https://produpdates.blob.core.windows.net/web/Pre-requsite%20Application-%202026/Prerequsite%20softwares/AdbeRdr11010_en_US.exe?sp=racw&st=2026-02-27T09:22:10Z&se=2031-02-27T17:37:10Z&spr=https&sv=2024-11-04&sr=b&sig=HE0hKTa6Cptcmbot6bSNLR7ThIS4KkfwX2cbGXECWzA%3D" -DestinationPath "$tempInstallPath\AdbeRdr11000_en_US.msi" -SoftwareName "Adobe Reader XI" -UninstallDisplayName "Adobe Reader XI"
Install-Package -Url "https://produpdates.blob.core.windows.net/web/Pre-requsite%20Application-%202026/Prerequsite%20softwares/dotnet-hosting-8.0.4-win.exe?sp=racw&st=2026-02-27T09:31:47Z&se=2031-02-27T17:46:47Z&spr=https&sv=2024-11-04&sr=b&sig=IOFkMN%2FXnkSPJwu6g4TpYq3PEJvbRXrAfAAB9CsLGs0%3D" -DestinationPath "$tempInstallPath\dotnet-hosting.exe" -SoftwareName "ASP.NET Core 8.0.4 Hosting" -InstallArgs @("/install", "/quiet", "/norestart") -UninstallDisplayName "Microsoft ASP.NET Core 8.0"
$accessDbUrl = if ([Environment]::Is64BitOperatingSystem) { "https://download.microsoft.com/download/3/5/C/35C84C36-661A-44E6-9324-8786B8DBE231/accessdatabaseengine_X64.exe" } else { "https://download.microsoft.com/download/3/5/C/35C84C36-661A-44E6-9324-8786B8DBE231/accessdatabaseengine.exe" }
...
# Output formatted table with custom host coloring based on status
