<#
.SYNOPSIS
    ProPhoenix Master Launcher (v10 - Stable)
    - Fixes "New-Object Point" layout crash.
    - Adds "-NoExit" to keep tool windows open for troubleshooting.
    - Contains exact user-provided logic for Hotfix and DB Sync tools.
    - Centralized Logging.
#>
...
# --- ACTION BUTTONS (RESIZED & ALIGNED) ---
...
$btnVer = New-Object System.Windows.Forms.Button; $btnVer.Text="Check Versions"; $btnVer.Location="270,550"; $btnVer.Size="250,45"; $btnVer.BackColor="DarkOrange"; $btnVer.FlatStyle="Flat"; $btnVer.Font=$fontHead; $btnVer.Enabled=$false
...
$lblStat = New-Object System.Windows.Forms.Label; $lblStat.Text="Ready."; $lblStat.Location="10,700"; $lblStat.AutoSize=$true; $lblStat.ForeColor="Yellow"
...
function Exec-Utility($actionName) {
    ...
    try {
        if ($IsLocal) {
            ...
            $s = New-PSSession -ComputerName $Target -ErrorAction Stop; $res = Invoke-Command -Session $s -ScriptBlock $RunAppMgrAction -ArgumentList $actionName, $false; Remove-PSSession $s; if ($res -match "SUCCESS") { Log-Write $res "Lime" } else { Log-Write $res "Red" } } } catch { Log-Write "Utility Error: $($_.Exception.Message)" "Red" } finally { Toggle-Inputs $true; $lblStat.Text = "Ready." } }
...
# --- SCRIPT 3: PD CAD (Cad_Hotfixupdate_v2.0.ps1) ---
$Content_CAD = @'
<#
.SYNOPSIS
...
# --- SETTINGS ---
$AppTitle = "ProPhoenix Master Launcher"

$form = New-Object System.Windows.Forms.Form
$form.Text = $AppTitle
$form.Size = New-Object System.Drawing.Size(600, 750) # Spacious Layout
$form.StartPosition = "CenterScreen"
$form.BackColor = [System.Drawing.Color]::FromArgb(30, 30, 30)
...
$BtnWidth = 450
$BtnSpacing = 30
$CenterX = ($form.Size.Width - $BtnWidth) / 2 - 10 
...
# --- PRE-CALCULATE POSITIONS (Fixes "Point" error) ---
...
function Launch-Script($scriptName, $fullPath) {
    if (Test-Path $fullPath) {
        ...
        try {
            # FIX: Added -NoExit to prevent instant closure
            Start-Process -FilePath "powershell.exe" -ArgumentList "-NoExit -ExecutionPolicy Bypass -File `"$fullPath`"" -Verb RunAs
